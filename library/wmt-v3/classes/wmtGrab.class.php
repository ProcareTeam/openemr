...
class Grab {
	...
	public $pf_token_url = 'Patient Form Url';

	public $ics_file_url = 'ICS File Url';	// Added Template tag
	...
	public function loadAppointment( $eid = false ) {
		...
		// ISC Link
		$icsLinkName = isset($GLOBALS['ICS_Link_Name']) && empty($GLOBALS['ICS_Link_Name']) ? 'ICS File' : $GLOBALS['ICS_Link_Name'];
		if(!empty($appt->ics_file) && isset($GLOBALS['ics_attachment']) && $GLOBALS['ics_attachment'] == "1") {
			$this->ics_file_url = '<a class="icsAttachmentlink" href="'. $appt->ics_file . '" target="_blank" style="display:none;">' . $icsLinkName . '</a>';
		}
