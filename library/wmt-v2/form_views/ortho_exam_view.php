...
if($data_exists) {
	...
	if($section_data_exists['ge']) {
		...
		$out = BuildPrintLine($out, 'Gait:', MultPrint($dt['oe_ge_gait'],'OrthGait',''));
	...
	if($section_data_exists['neu']) {
		...
		$out = BuildPrintLine($out, 'Sensory Cervical - C3 (Supraclavicular fossa)',MultPrint($dt['oe_neu_sensory_cervical_c3'],'Sensory_Cervical'));
		$out = BuildPrintLine($out, 'Sensory Cervical - C4 (Posterior shoulder)',MultPrint($dt['oe_neu_sensory_cervical_c4'],'Sensory_Cervical'));
		$out = BuildPrintLine($out, 'Sensory Cervical - C5 (Lateral upper arm)',MultPrint($dt['oe_neu_sensory_cervical_c5'],'Sensory_Cervical'));
		$out = BuildPrintLine($out, 'Sensory Cervical - C6 (Thumb tip)',MultPrint($dt['oe_neu_sensory_cervical_c6'],'Sensory_Cervical'));
		$out = BuildPrintLine($out, 'Sensory Cervical - C7 (3rd finger tip)',MultPrint($dt['oe_neu_sensory_cervical_c7'],'Sensory_Cervical'));
		$out = BuildPrintLine($out, 'Sensory Cervical - C8 (5th finger tip)',MultPrint($dt['oe_neu_sensory_cervical_c8'],'Sensory_Cervical'));

		$out = BuildPrintLine($out, 'Sensory Lumbar - L1 (Inguinal-Upper Thigh)',MultPrint($dt['oe_neu_sensory_lumbar_l1'],'Sensory_Lumbar'));
		$out = BuildPrintLine($out, 'Sensory Lumbar - L2 (Mid to upper anterior thigh)',MultPrint($dt['oe_neu_sensory_lumbar_l2'],'Sensory_Lumbar'));
		$out = BuildPrintLine($out, 'Sensory Lumbar - L3 (Medial lower thigh)',MultPrint($dt['oe_neu_sensory_lumbar_l3'],'Sensory_Lumbar'));
		$out = BuildPrintLine($out, 'Sensory Lumbar - L4 (Medial calf/ankle)',MultPrint($dt['oe_neu_sensory_lumbar_l4'],'Sensory_Lumbar'));
		$out = BuildPrintLine($out, 'Sensory Lumbar - L5 (Dorsum of foot/great toe web space)',MultPrint($dt['oe_neu_sensory_lumbar_l5'],'Sensory_Lumbar'));
		$out = BuildPrintLine($out, 'Sensory Lumbar - S1 (Lateral foot/heel)',MultPrint($dt['oe_neu_sensory_lumbar_s1'],'Sensory_Lumbar'));
	...
	if($section_data_exists['orth']) {
		...
		$out = BuildPrintLine($out, 'Femoral Stretch:',MultPrint($dt['oe_orth_femoral'],'OrthFemStr'));
		if($out) PrintSingleLine('', $out);
	...
	if($section_data_exists['palp']) {
		...
		$out = '';
		$out = BuildPrintLine($out,'Cervical Alignment:',MultPrint($dt['oe_palp_cerv'],'NormAbnorm1'));
		$out = BuildPrintLine($out,'Thoracic Alignment:',MultPrint($dt['oe_palp_thor'],'NormAbnorm1',''));
		$out = BuildPrintLine($out,'Lumbar Alignment:',MultPrint($dt['oe_palp_lum'],'NormAbnorm1'));
		$out = BuildPrintLine($out,'Fingertip Test:',MultPrint($dt['oe_palp_fin'],'OrthoFT'));
		$out = BuildPrintLine($out,'Hip:',MultPrint($dt['oe_palp_hip'],'OrthoHip'));
		if($out) PrintSingleLine('', $out);
	...
	if($section_data_exists['msc']) {
		...
		$out = '';
		$out = BuildPrintLine($out, 'C5 (Elbow flex, palm up - brachioradialis):',MultPrint($dt['oe_msc_c5'],'OrthoDerm'));
		$out = BuildPrintLine($out, 'C6 (Elbow flex, thumbs up, biceps):',MultPrint($dt['oe_msc_c6'],'OrthoDerm'));
		$out = BuildPrintLine($out, 'C7 (Elbow extension):',MultPrint($dt['oe_msc_c7'],'OrthoDerm'));
		$out = BuildPrintLine($out, 'C8 (Long finger extension):',MultPrint($dt['oe_msc_c8'],'OrthoDerm'));
		$out = BuildPrintLine($out, 'C8, T1 (Finger Abduction):',MultPrint($dt['oe_msc_c8t1'],'OrthoDerm'));
		$out = BuildPrintLine($out, 'L1, L2, L3 (Hip flexion, iliopsoas):',MultPrint($dt['oe_msc_l1'],'MuscleTest'));
		$out = BuildPrintLine($out, 'L2, L3, L4 (Knee extension, Quadriceps femoris):',MultPrint($dt['oe_msc_l2'],'MuscleTest'));
		$out = BuildPrintLine($out, 'L4 (Foot inversion, Tibialis anterior):',MultPrint($dt['oe_msc_l4'],'MuscleTest'));
		$out = BuildPrintLine($out, 'L5, S1 (Great toe extension, Extensor hallucis longus):',MultPrint($dt['oe_msc_l5'],'MuscleTest'));
		$out = BuildPrintLine($out, 'S1, S2 (Ankle Plantarflexion, Gastrocnemius, Soleus):',MultPrint($dt['oe_msc_s1s2'],'MuscleTest'));
