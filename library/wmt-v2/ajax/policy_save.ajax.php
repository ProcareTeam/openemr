...
if(isset($updateallpayer) && $updateallpayer == true) {
    ...
    for ($i=1; $i <= $payercount ; $i++) {
        ...
        $update_payer = filter_input(INPUT_POST, "i".$i."updatepayer");
        ...
        if(empty($effective_date)) {
            ...
            $eList = array();
            while($eRow = sqlFetchArray($eData)) { 
                if(isset($eRow['date']) && !empty($eRow['date'])) { 
                    $eList[] = strtotime($eRow['date']); 
                }

                // Check if payer already exist 
                if(isset($eRow['id']) && !empty($eRow['id']) && $eRow['id'] == $payerid) {
                    $effective_date = $eRow['date'];
                }
            }
