...
if($form_mode == 'update') {
  ...
  if($encounter == '') $encounter = date('Ymd');
	...
	if(!$id || $id == '') {
  	...
  	/* OEMR */
  	$form_ufa = isset($_REQUEST['form_ufa']) ? $_REQUEST['form_ufa'] : 0;
  	if($form_ufa === "1") {
			sqlStatement("UPDATE openemr_postcalendar_events ope set ope.pc_case = ? where cast(concat(ope.pc_eventDate, ' ', ope.pc_startTime) as datetime) >= CURDATE() and ope.pc_pid = ?", array($id, $pid));

			$ufeData = sqlStatement("SELECT cal.id from form_encounter fe left join case_appointment_link cal on cal.encounter = fe.encounter WHERE fe.`date` >= CURDATE() and fe.pid = ?", array($pid));
			while ($uferow = sqlFetchArray($ufeData)) {
				sqlStatement("UPDATE case_appointment_link cal set cal.enc_case = ? WHERE cal.id = ? and cal.pid = ?", array($id, $uferow['id'], $pid));
			}
  	}
