...
if($diag_use_ajax) {
	...
	$show_curr_btn = "<a class='css_button' tabindex='-1' onclick='return ajaxIssueRefresh(\"$encounter\",\"$pid\",\"$frmdir\",\"$id\",\"medical_problem\",\"current\",\"Diagnoses / Problems\",\"$target_container\",\"dg_\",\"$wrap_mode\",\"".$dt['tmp_diag_window_mode']."\");' href='javascript:;'><span>Select from Current Diagnosis</span></a>";
	...
	$show_curr_btn = "<a class='css_button' tabindex='-1' onclick='return SetSelectCurrentDiagnosis(\"$base_action\",\"$wrap_mode\",\"$id\",\"current\",\"$encounter\");' href='javascript:;'><span>Select from Current Diagnosis</span></a>";
...
if(count($diag) > 0) {
	foreach($diag as $prev) {
		...
		$dg_action_mode = "";
		if(isset($dt['tmp_diag_window_mode']) && $dt['tmp_diag_window_mode'] == "current") {
			if($prev['encounter'] != $encounter) {
				$dg_action_mode = "current";
			}
		}
		...
		if($first_pass && $form_mode == 'new' && !$prev['seq'] && $use_sequence) {
   		...
   		echo "<tr>\n";
   		echo "<td class='$suppress_class wmtLabel'>";
		if($use_sequence) {
			echo "<input name='dg_id_$cnt' id='dg_id_$cnt' type='hidden' readonly='readonly' value='".$prev['id']."' />#)&nbsp;<input name='dg_seq_$cnt' id='dg_seq_$cnt' class='wmtInput' type='text' style='width: 40px;' value='".htmlspecialchars($prev['seq'],ENT_QUOTES)."' /></td>\n";
		} else {
			echo "<input name='dg_id_$cnt' id='dg_id_$cnt' type='hidden' readonly='readonly' value='".$prev['id']."' />&nbsp;$cnt&nbsp;).&nbsp;";
		}

		// if(!empty($dg_action_mode)) {
		// 	echo "<input name='dg_action_mode_$cnt' id='dg_action_mode_$cnt' type='hidden' readonly='readonly' value='".$dg_action_mode."' />";
		// }
...
echo "		<td>";
echo "		<input name='tmp_diag_cnt' id='tmp_diag_cnt' type='hidden' value='".($cnt-1)."' />";
echo "		<input name='tmp_current_diagnosis' id='tmp_current_diagnosis' type='hidden' value='".json_encode($current_diagnosis_codes)."' />";
echo "		&nbsp;</td>\n";
