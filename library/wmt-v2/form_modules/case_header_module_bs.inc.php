...
$local_fields = array('case_description', 'prior_auth_number',
	...
	'accident_state', 'accident_type', 'other_accident', 'employment_related', 'illness',
	...
	'case_guarantor_pid', 'employer', 'similar_symptoms', 'case_dt', 'notes', 'lb_date', 'lb_notes', 'auth_req', 'auth_start_date', 'auth_end_date', 'auth_num_visit', 'auth_provider', 'bc_date', 'bc_notes', 'bc_notes_dsc', 'bc_stat', 'liability_payer_exists', 'auth_notes', 'vh_case_manager');
...
if($GLOBALS['wmt::limit_case_ins']) {
	...
	$policies = wmtPatData::getPidPolicies($pid, 'ins.`id` DESC');
}
$cnt = 1;
unset($ins);
unset($ins_data);
...
<div>
  ...
  <div class="form-row">
	...
	<div class="form-group col-md-3">
    	<label><?php echo xl('Accident Type'); ?>:</label>
      	<select name="<?php echo $field_prefix; ?>accident_type" id="<?php echo $field_prefix; ?>accident_type" class="form-control">
		  <?php ListSel($dt[$field_prefix . 'accident_type'], 'accident_type'); ?>
		</select>
    </div>
  </div>
...
<script type="text/javascript">
	...
	window.formScriptValidations.push(async() => await caselibObj.validate_InsStatusVerify('<?php echo $pid; ?>'));
</script>
...
$(document).ready(function(){
    ...
    $("#add_policy_btn").on('click', function(e) {
        ...
        dlgopen(url, 'policyPop', 'modal-lg', '800', '', title);
			/*
