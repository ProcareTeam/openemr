...
class wmtCase {
    ...
    public static function savePICaseManagmentDetails($case_id = '', $data = array(), $isActive = 1) {
                ...
                if(!empty($dk)) {
                    ...
                    if(is_array($dItem)) {
                        foreach ($dItem as $diK => $dsItem) {
                            if(!empty($dsItem)) {
                                //Insert Items
                                $insertSql = "INSERT INTO `vh_pi_case_management_details` (case_id, field_name, field_index, field_value, isActive) VALUES (?, ?, ?, ?, ?) ";
    ...
    public static function getActionItems($case_id = '') {
        $resultItem = array();
        if(!empty($case_id)) {
            $result = sqlStatement("SELECT * from vh_action_items_details where case_id = ? order by id asc", $case_id);
            while ($row = sqlFetchArray($result)) {
                $resultItem[] = $row;
            }
        }
