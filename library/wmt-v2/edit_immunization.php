...
if ($_POST['form_save']) {
	...
	if ($form_data->id) {
		...
		$form_data->updated_by = $_SESSION['authUserID'];
		$form_data->update();
	}
	else {
		$form_data->create_date = date('Y-m-d H:i:s');
		$form_data->created_by = $_SESSION['authUserID'];
				...
				<tr>
					...
					<td class="wmtData">
						<select class="wmtFullInput" name="form_admin_by">
							<option value="">&nbsp;</option>
<?php 
	if (!$form_data->id) $form_data->administered_by = $_SESSION['authUserID']; // for new records
