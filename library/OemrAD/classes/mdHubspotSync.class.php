...
class HubspotSync {
	...
	public static function hubspotSync($type = 0, $itemIds = array()) {
		$totalItem = 0;
		$totalsentItem = 0;
		$event_type = (isset($type) && $type !== 0) ? $type : false;
	...
	public static function handleInCreateData($hId = '', $param = array(), $opt = array()) {
		...
		$rData = array(
			'sent' => 2,
			'sent_time' => date('Y-m-d H:i:s'),
			'status' => "This functionality disable (handleInCreateData)",
			'request_body' => ''
		);
	...
	public static function prepareOutData($data = array()) {
		$pData = array();
	...
	public static function getCompanyData($objectId, $options = array()) {
		if(isset($objectId) && !empty($objectId)) {
			...
			$contactAssociationsRes = self::callRequest(
				'', 
				array(
					"url" => "https://api.hubapi.com/crm/v3/objects/contacts/$objectId/associations/company?limit=500" . $extraProp,
					"method" => "GET",
				),
				array(
					'bearer_token' => $bearer_token
				...
				if(count($contactAssociationsRes['results']) > 0) {
					...
					if(isset($assocItem['id'])) {
						$companyRes = self::callRequest(
							'', 
							array(
								"url" => "https://api.hubapi.com/crm/v3/objects/companies/".$assocItem['id']."?archived=false" . $extraProp ,
								"method" => "GET",
							),
							array(
								'bearer_token' => $bearer_token
	...
	public static function getAllContactData($options = array()) {
		...
		$contactResponce = self::callRequest(
			'', 
			array(
				"url" => "https://api.hubapi.com/crm/v3/objects/contacts?properties=email&properties=firstname&properties=lastname&properties=company&properties=jobtitle&properties=fax&properties=phone&properties=mobilephone&properties=address&properties=city&properties=state&properties=zip&properties=country" . $extraProp,
				"method" => "GET",
			),
			array(
				'bearer_token' => $bearer_token
	...
	public static function updateContactData($objectId = '', $payload = array(), $options = array()) {
		if(isset($payload) && !empty($payload) && !empty($objectId)) {
			...
			$contactResponce = self::callRequest(
				$payload, 
				array(
					"url" => "https://api.hubapi.com/crm/v3/objects/contacts/$objectId",
					"method" => "PATCH",
					"header" => ["Content-Type: application/json", "Accept: application/json"]
				),
				array(
	...
	public static function createlogMappingData($data = array()) {
		extract($data);
	...
	public static function callRequest($body, $config = array(), $opt = array()) {
		if(isset($config["method"])) {
			//if($config["method"] == "GET") {
    			...
    			if(isset($opt["bearer_token"]) && !empty($opt["bearer_token"])) {
    				$authorization = "Authorization: Bearer ".$opt["bearer_token"];
