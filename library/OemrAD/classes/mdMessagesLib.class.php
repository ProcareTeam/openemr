...
class MessagesLib {
	...
	public static function add_files_list() {
		...
		self::saveGPrelations($note_id, $filesDocList);
	}
	...
	public static function linked_doc_list() {
        ...
        if (sqlNumRows($tmp)) {
            ...
            while ($gprow = sqlFetchArray($tmp)) {
            	...
            	if(!empty($msgData)) {
            		$msgType = $msgData['type'] == "SMS" || $msgData['type'] == "EMAIL" ? $msgData['type'] . "_REPLY" : $msgData['type'];
	...
	public static function getDocumentData($docId) {
	    ...
	    $query = "SELECT d.id, d.type, d.size, d.url, d.docdate, d.list_id, d.encounter_id, d.foreign_id, d.name as doc_name, c.name " .
	    ...
	    if(!empty($dres)) {
		    $dres['baseName'] = $dres['doc_name'] . ' (' . xl_document_category($dres['name']). ')';
		    $dres['baseFileName'] = ' (' . xl_document_category($dres['name']). ')' . $dres['doc_name'];
	...
	public static function displayAttachment($type, $id, $data, $setPatient = true) {
		global $webserver_root;
					...
					<?php
						foreach ($docsList as $key => $doc) {
							...
							if(isset($doc['doc_id']) && !empty($doc['doc_id'])) {
								?>
								<li>
									<a href="<?php echo $GLOBALS['webroot']. "/controller.php?document&retrieve&patient_id=".$pid."&document_id=". $doc['doc_id'] ?>"><?php echo $doc['file_name']; ?></a> - <a href="javascript:void(0)" style="cursor: pointer;" class="docrow" id="<?php echo $doc['doc_id']; ?>" onClick="handlegotoReport('<?php echo $doc['doc_id']; ?>', '<?php echo $pid; ?>', <?php echo var_export($setPatient,true); ?>)" >(View)</a>
								</li>
								 <?php
							} else {
								?>
	...
	public static function addDocuments($type, $list, $file_name, $pid, $category_id, $doc_date = '') {
        if(!empty($list)) {
    ...
    public static function displayIframeMsg($msg = '', $body_class = '', $msgV = 'TEXT/PLAIN') {
		...
		if($msgV === 'TEXT/PLAIN') $formatedMessage = str_replace("\r\n","<br/>",$formatedMessage);
		
		$formatedMessage = '<html><body class="'.$body_class.'">'.$formatedMessage.'</body></html>';
		
		if($msgV === 'TEXT/PLAIN') $styleCss = '<link rel="stylesheet" href="'.$GLOBALS['webroot'].'/public/themes/style_light.css?v=71" type="text/css">';
	...
	public static function replaceMessageContent($message_content, $msg_id = "") {
		$attachmentList = array();
		
		if(!empty($msg_id)) {
			$attachmentList = MessagesLib::getMsgDocs($msg_id);
		}

		if(!empty($attachmentList)) {

			preg_match_all('/alt="(.*)".*src="cid:(.*)"/Uims', $message_content, $matches1);
            if(count($matches1)) {
            	$search1 = array();
                $replace1 = array();

                foreach($matches1[2] as $mi => $match) {
                	$search1[] = "src=\"cid:$match\"";

                	foreach ($attachmentList as $ai) {
                		if(isset($ai['file_name']) && $ai['file_name'] == $matches1[1][$mi]) {
                			$replace1[] = "src='" . $GLOBALS['webroot'] . "/interface/main/attachment/downloadDoc.php?view=1&path=" . $GLOBALS['webroot'] . $ai['url'] . "&name=" . $ai['file_name'] . "'";
                			break;
                		}
                	}
                }

                if(!empty($search1) && !empty($replace1)) {
            		$message_content = str_replace($search1, $replace1, $message_content);
            	}
            }

            preg_match_all('/src="cid:(.*)".*alt="(.*)"/Uims', $message_content, $matches1);
            if(count($matches1)) {
            	$search1 = array();
                $replace1 = array();
                ...
                foreach($matches1[1] as $mi => $match) {
                	...
                	foreach ($attachmentList as $ai) {
                		if(isset($ai['file_name']) && $ai['file_name'] == $matches1[2][$mi]) {
                			$replace1[] = "src='" . $GLOBALS['webroot'] . "/interface/main/attachment/downloadDoc.php?view=1&path=" . $GLOBALS['fileroot'] . $ai['url'] . "&name=" . $ai['file_name'] . "'";
                			break;
                		}
                	}
                }
            ...
            preg_match_all('/\[image: (.*)\]/Uims', $message_content, $matches2);
            if(count($matches2)) {
            	$search2 = array();
                $replace2 = array();
                ...
                foreach($matches2[1] as $match) {
                	...
                	foreach ($attachmentList as $ai) {
                		if(isset($ai['file_name']) && $ai['file_name'] == $match) {
                			$replace2[] = "<img alt='" . $ai['file_name']. "' src='" . $GLOBALS['webroot'] . "/interface/main/attachment/downloadDoc.php?view=1&path=" . $GLOBALS['fileroot'] . $ai['url'] . "&name=" . $ai['file_name'] . "' />";
                			break;
                		}
                	}
                }

                if(!empty($search2) && !empty($replace2)) {
            		$message_content = str_replace($search2, $replace2, $message_content);
            	}
