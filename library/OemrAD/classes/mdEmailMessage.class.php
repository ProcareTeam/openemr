...
class EmailMessage {
	...
	public static function getAttchmentDocumentList($id) {
		$queryarr = array($pid);
	    $query = "SELECT * " .
	...
	public static function headMessageContent($pid) {
		?>
		<style type="text/css">
	 ...
	 */
	public static function logEmail($event, $toNumber, $fromNumber, $pid, $msgId, $timestamp, $msg_status, $message, $direction='in', $active=true, $receivers_name = "", $message_subject = "", $raw = null) {
	...
	public static function filterMsg(&$data) {
	...
	public static function getAttachments($connection, $emailIdent, $structure, $prefix = '', $pindex = 1) {
		$attachments = array();
		$nattachments = array();
			...
			for($i = 0; $i < count($structure->parts); $i++) {
				...
				if($structure->parts[$i]->type > 2) {
					if(isset($structure->parts[$i]->id)) {
						$id = str_replace(array('<', '>'), '', $structure->parts[$i]->id);
						$attachments[$i]['name'] = EmailReader::getFilenameFromPart($structure->parts[$i]);
						$attachments[$i]['subtype'] = $structure->parts[$i]->subtype;
						$attachments[$i]['inline'] = true;
						$attachments[$i]['file_id'] = $id;
						$attachments[$i]['is_attachment'] = true;
					} else if(isset($structure->parts[$i]->disposition) && $structure->parts[$i]->disposition == "ATTACHMENT") {
					...
					} else if(isset($structure->parts[$i]->disposition) && $structure->parts[$i]->disposition == "INLINE") {
					...
					if($attachments[$i]['is_attachment'] === true) {
						...
						$attachments[$i]['attachment'] = imap_fetchbody($connection, $emailIdent, $partNumber);
				...
				if(isset($structure->parts[$i]->parts)) {
					$na = self::getAttachments($connection, $emailIdent, $structure->parts[$i], $prefix . ($i + 1).'.');
					if(!empty($na)) {
						foreach ($na as $nai) {
							$nattachments[] = $nai;
						}
		...
		if(isset($nattachments) && !empty($nattachments)) {
			$attachments = array_merge($attachments, $nattachments);
		}
	...
	public static function getImapConnection() {
		try {
			...
			$imapPass = $cryptoGen->decryptStandard($GLOBALS['IMAP_PASS']);

			return imap_open($GLOBALS['IMAP_SERVER_URL'], $GLOBALS['IMAP_USER'], $imapPass);
		} catch (Exception $e) {
			...
			try {
			    ...
			    if (!empty($emailData)) {
	        		...
	        		$mail = EmailReader::_fetchHeader1($header1, $mail);
							...
							$mailJSON = json_encode(array('mail' => $mail, 'encoding' => $structure->encoding));
			        ...
			        /*Info*/
		        		...
		        		/*Decode content if encoded*/
					  		...
					  		if ($obj_section->encoding == 0) {
								    ...
								    $message_content = quoted_printable_decode(imap_8bit($message_content));
								} else if ($obj_section->encoding == 2) {
								    $message_content = imap_binary($message_content);
								} else if ($obj_section->encoding == 3) {
								    $message_content = imap_base64($message_content);
								} else if ($obj_section->encoding == 4) {
								    $message_content = imap_qprint($message_content);
								} else {
										$message_content = imap_utf8($message_content);
								}
	...
	/*Imap help to fetch incoming email*/
	public static function resyncEmailContent($msgid = '') {
		
		$msgData = sqlQuery("SELECT * from message_log ml where id = ? order by id desc;", array($msgid));
		$status = array();

		if(!empty($msgData) && $msgData['type'] == 'EMAIL' && $msgData['direction'] == 'in') {
			$msgRawData = isset($msgData['raw_data']) ? $msgData['raw_data'] : "";
			$mailObj = self::isJson($msgRawData) ? json_decode($msgRawData) : array();
			$oldmail = isset($mailObj->mail) ? $mailObj->mail : array();
			...
			try {
				...
				if(empty($msgid)) {
					throw new \Exception("Emtpy message id");
				}
				...
				$ondate = !empty($oldmail->timestamp) ? date("j F Y", $oldmail->timestamp) : date("j F Y", strtotime($msgData['msg_time']));
				//$esubject = !empty($oldmail->subject) ? $oldmail->subject : $msgData['message_subject'];
				//$efrom = !empty($oldmail->from) ? $oldmail->from : $msgData['msg_from'];
				//$eto = !empty($oldmail->to) ? $oldmail->to : $msgData['msg_to'];

				//Temp
				$esubject = $msgData['message_subject'];
				$efrom = $msgData['msg_from'];
				$eto = $msgData['msg_to'];
				$etimestamp = strtotime($msgData['msg_time']);

				$emailSearchStr = array();
				if(!empty($esubject)) $emailSearchStr[] = 'SUBJECT "'.trim($esubject).'"';
				if(!empty($efrom)) $emailSearchStr[] = 'FROM "'.trim($efrom).'"';
				if(!empty($eto)) $emailSearchStr[] = 'TO "'.trim($eto).'"';
				//if(!empty($ondate)) $emailSearchStr[] = 'ON "'.trim($ondate).'"';

				if(empty($emailSearchStr)) {
					throw new \Exception("Empty search string");
				}

				$emailSearchStr = implode(" ", $emailSearchStr);

	      //$emailData = imap_search($connection, 'SUBJECT "'.$oldmail->subject.'" FROM "'.$oldmail->from.'" TO "'.$oldmail->to.'" ON "'.date("j F Y", $oldmail->timestamp).'"');
	      $emailData = imap_search($connection, $emailSearchStr);

	      if(!empty($emailData) && count($emailData) >= 1) {
	      	...
	      	foreach ($emailData as $emailIndex => $emailIdent) {
				  		...
				  		$mail = EmailReader::_fetchHeader1($header1, $mail);
							...
							if($mail->timestamp == $etimestamp && $toaddr == $eto && $fromaddr == $efrom) {

								$status[] = "Message found - Done";
					  		...
					  		if ($obj_section->encoding == 0) {
								...
								if(empty($message_content)) {
									throw new \Exception("Something went wrong with message content");
								}

								if(!empty($msgid)) {
		 							$msgdoc = sqlStatement("SELECT * FROM message_attachments WHERE message_id=?", array($msgid));
									while($msgdocrow = sqlFetchArray($msgdoc)) {
										$msgdocfilePath = !empty($msgdocrow['url']) ? $GLOBALS["fileroot"] . $msgdocrow['url'] : '';
										if (!empty($msgdocfilePath) && file_exists($msgdocfilePath)) {
	                      unlink($msgdocfilePath);
	                      $status[] = "Old attachment deleted (".$msgdocrow['file_name'].") - Done";
	                  }
									}

									//Delete all attachment before delete.
									sqlStatementNoLog("DELETE FROM `message_attachments` WHERE message_id = ?", array($msgid));

									$attachmentList = self::getAttachments($connection, $emailIdent, $structure);
									$attachmentList = self::writeFile($attachmentList, $emailIdent);

									//Save document
									if(isset($msgid) && !empty($msgid) && isset($attachmentList)) {
										foreach ($attachmentList as $key => $item) {
											self::writeMessageDocumentLog($msgid, "file_url", $item['file_name'], $item['url']);
											$status[] = "New attachment saved (".$item['file_name'].") - Done";
										}
									}
									
									//Update message log message content
									sqlStatementNoLog("UPDATE `message_log` SET `message` = ?, `raw_data` = ? WHERE id = ?", array($message_content, $mailJSON, $msgid));

									$status[] = "Message content updated - Done";
								}
	      	...
	      	$status[] = "More then 1 email in the inbox or not found - Error";
	      }
      } catch(\Exception $e) {
				$status[] = $e->getMessage();
			}
	...
	public static function getIncomingEmail($pid = '', $sync = "false") {
	...
	public static function fetchEmailOnMessageBoard($pid = '', $sync = "false") {
    ...
    $pParam["FIND_IN_SET('".$formattedfromaddr."', secondary_email)"] = array("condition" => "OR");
    $pParam["phone_cell"] = array("value" => $msg_from, "condition" => "OR");

    $result = getPatientByCondition($pParam, "COUNT(pid) as count");
	...
	public static function isFailedToSend($data) {
		$isFailed = false;
	...
	public static function isFailedToSMSSend($data) {
		$isFailed = false;
    ...
    try {
        ...
        foreach ($eData as $ek => $emailData) {
            ...
            try {
                ...
                // ICS
                $dowloadics = isset($GLOBALS['ics_attachment']) ? $GLOBALS['ics_attachment'] : "";
                $emailBody = isset($email_data['html']) ? $email_data['html'] : "";

		        		if(!empty($emailBody) && $dowloadics == "1") {
			        		preg_match_all('/class="icsAttachmentlink" href="(.*)"/Uims', $emailBody, $matches1);
			        		$external_files_list = array();

			            if(count($matches1) > 0 && $dowloadics == "1") {
		                foreach($matches1[1] as $mi => $match) {
		                	if(isset($emailData['request_data']) && isset($emailData['request_data']['external_files']) && count(json_decode($emailData['request_data']['external_files'], true)) > 0) {
				            		continue;
				            	}
		                ...
		                if(!empty($external_files_list)) {
		                	$emailData['request_data']['external_files'] = json_encode($external_files_list);
		              	}
			            }
		          	}
		        		// END
                ...
                //Clear Temp document files
                $attachmentList = Attachment::clearTempAttachmentFile($attachmentList);
            ...
            for ($ei=0; $ei < count($emailItems); $ei++) {
                try {
                    ...
                    if(!empty($emailData['request_data'])) {
                        $arrayTypeList = array("local_files", "documents", "notes", "orders", "encounter_forms", "demos_insurances", "external_files");
          ...
          if(!empty($email)) {
              foreach ($attachmentList as $aKey => $aItem) {
                  if(isset($aItem['type']) && !empty($aItem['type'])) {
                      ...
                      } else if((isset($aItem['url']) && !empty($aItem['url'])) && (isset($aItem['file_name']) && !empty($aItem['file_name']))) {
                      		$email->addStringAttachment(file_get_contents($aItem['url']), $aItem['file_name'], 'base64', $aItem['file_type']);
                      }
                  }
              }
          }
