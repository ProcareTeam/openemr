...
class IdempiereWebservice {
	...
	public static function getActionConfiguration($id = '', $type = '', $actionType = '') {
		$result_list = array();
		$binds = array();
	...
	public static function syncInfoToIdempiere($type = 0, $itemIds = array()) {
		$totalItem = 0;
		$totalsentItem = 0;
		$event_type = (isset($type) && $type !== 0) ? $type : false;
	...
	public static function updatePrepared($id, $data) {
		if(!empty($data) && !empty($id)) {
			$binds = array();
			$setColsList = array();
	...
	public static function callRequest($body, $config = array()) {
		...
		try {
			...
			if (!file_exists(dirname( __FILE__, 2 ) . "/soap/lib/nusoap.php")) {
			...
			if(isset($config) && !empty($body)) {
				if (file_exists(dirname( __FILE__, 2 ) . "/soap/lib/nusoap.php")) {
					require_once(dirname( __FILE__, 2 ) . "/soap/lib/nusoap.php");
	...
	public static function loopDataSet($req, $items = array()) {
		$dataSet = array();
