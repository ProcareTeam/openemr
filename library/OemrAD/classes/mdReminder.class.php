...
class Reminder {
	...
	public static function isDataEmpty($data) {
		...
		} else if($data && isset($data['action_type']) && ( $data['action_type'] == "inmoment" || $data['action_type'] == "wordpress" )) {
			$field_list = array('id', 'seq', 'trigger_type');
			foreach ($data as $key => $value) {
				if(in_array($key, $field_list)) {
					if($key == "trigger_type") {
	...
	public static function isActionEventDataEmpty($data) {
		$isEmpty = false;
	...
	public static function getTestNotificationConfiguration() {
		$result_list = array();
		$binds = array(1);
				...
				if(isset($item['config_id']) && !empty($item['config_id'])) {
					...
					if(isset($configs) && count($configs) > 0) {
						foreach ($configs as $key => $config) {
							...
							/*
							if($config['action_type'] == "messaging" && $item['tablename'] == "openemr_postcalendar_events" && !empty($item['uniqueid']) && !empty($item['id'])) {

								$aData = sqlQueryNoLog("SELECT ope.pc_eid, concat(ope.pc_eventDate,' ',ope.pc_startTime) as event_datetime from openemr_postcalendar_events ope where pc_eid = ? order by pc_eid desc", array($item['uniqueid']));

								if(!empty($aData)) {
									$tTime = !empty($item['trigger_time']) ? strtotime($item['trigger_time']) : "";
									$aTime = !empty($aData['event_datetime']) ? strtotime($aData['event_datetime']) : "";

									if(!empty($tTime) && !empty($aTime) && $aTime < $tTime) {
										//Update status
										sqlStatementNoLog("UPDATE notif_log SET sent = 10 where id = ? ", array($item['id']));
									}
								}
							}*/
						}	
	...
	public static function sendIntMsgNotification($type = 0, $itemIds = array()) {
		$exceptionList = array();
		$totalItem = 0;
		$totalsentItem = 0;
		$event_type = (isset($type) && $type !== 0) ? $type : false;
	...
	public static function sendApiData($type = 0, $itemIds = array()) {
		$exceptionList = array();
		$totalItem = 0;
		$totalsentItem = 0;
		$event_type = (isset($type) && $type !== 0) ? $type : false;
	...
	public static function sendNotification($type = 0, $itemIds = array()) {
		$exceptionList = array();
		$totalItem = 0;
		$totalsentItem = 0;
		$event_type = (isset($type) && $type !== 0) ? $type : false;
					...
					if(!empty($item['trigger_time'])) {
						...
						if($item['sent'] == 0 && $trigger_unix_time <= $current_unix_time) {
							...
							if($item['tablename'] == "openemr_postcalendar_events" && !empty($item['uniqueid'])) {
								$apptData = sqlQuery("SELECT ope.pc_eid, TIMESTAMP(ope.pc_eventDate, ope.pc_startTime) as eventDateTime from openemr_postcalendar_events ope where pc_eid = ? ", array($item['uniqueid']));
								if(isset($apptData['eventDateTime']) && !empty($apptData['eventDateTime'])) {
									$appt_event_unix_time = strtotime($apptData['eventDateTime']);
									if($current_unix_time > $appt_event_unix_time) {
										self::updatePreparedData($item['id'], array(
											'sent' => 4
										));
										$totalItem++;
		...
		if(!empty($data['pid'])) {
			...
			if($fullAddress['status'] == true) {
				...
				try {
			        ...
			        $postal_letter_data['address'] = $from_address;
			        $postal_letter_data['address_json'] = json_encode($from_address_json);
			        $postal_letter_data['reply_address'] = $from_reply_address;
			        $postal_letter_data['reply_address_json'] = $from_reply_address_json;
	...
	public static function prepareInternalNotificationData($prepareFor = 'both', $eventid_param = '', $configid_param = '') {
		$exceptionList = array();
													...
													while ($result_data = sqlFetchArray($result)) {
														...
														if(!empty($pc_id) && !empty($tablename) && !empty($uniqueid)) {
															...
															if($isExists === false) {
																...
																if($config['action_type'] == "messaging" && isset($config['patient_form']) && !empty($config['patient_form'])) {

																	$patientFormShortenUrl = "";
																	$patientFormDataId = "";

																	try {
																		@$patientForm = new FormController();

																		$cStatus = $patientForm->checkMessagingStatus($pc_id, $config['communication_type']);

																		if($cStatus === true) {
																		$ftv = $patientForm->getFormIdType($config['patient_form']);

																		if(isset($ftv['formId']) && $ftv['formType']) {
																			$pt_formId = $ftv['formId'];
																			$pt_formType = $ftv['formType'];
																		}

																		if(isset($pt_formId) && isset($pt_formType)) {

																			@$patientFormData = $patientForm->sendFormToken($pt_formId, "getlink", $pt_formType, $pc_id, "link", "", "", true);

																			if(!empty($patientFormData) && isset($patientFormData['shortenUrl']) && !empty($patientFormData['shortenUrl'])) {
																				$patientFormShortenUrl = $patientFormData['shortenUrl'];
																			}

																			if(!empty($patientFormData) && isset($patientFormData['form_data_id']) && !empty($patientFormData['form_data_id'])) {
																				$patientFormDataId = $patientFormData['form_data_id'];
																			}
																		}
															        ...
															        global $pf_token_url;
                        											$pf_token_url = $patientFormShortenUrl;

															        //$message_text = str_replace('{{token_url}}', $patientFormShortenUrl, $message_text);
																...
																if(!empty($triggerTime)) {
																	...
																	$nId = self::savePreparedData($preparedData);
																	$totalPreparedItem++;

																	$preparedItemStatus = self::prepareItemStatus($config, $preparedItemStatus);

																	// Patient form reference
																	if(isset($patientFormDataId) && !empty($patientFormDataId) && !empty($nId)) {
																		
																		$patientForm->saveReminderFormReminderlog(
																			array(
																				"form_data_id" => $patientFormDataId,
																				"table_name" => "notif_log",
																				"uniqueid" => $nId
																			));
																	}
																}
			...
			foreach ($data as $key => $item) {
				...
				if($appStatus === true) {
					...
					if($config['action_type'] == "messaging" && isset($config['patient_form']) && !empty($config['patient_form'])) {
						...
						try {
							@$patientForm = new FormController();
							$cStatus = $patientForm->checkMessagingStatus($item['pid'], $config['communication_type']);
							...
							@$patientFormData = $patientForm->sendFormToken($pt_formId, "getlink", $pt_formType, $item['pid'], "link", "", "", true);
				...
				// Patient form reference
				if(isset($patientFormDataId) && !empty($patientFormDataId) && !empty($item['id'])) {
					$patientForm->saveReminderFormReminderlog(
						array(
							"form_data_id" => $patientFormDataId,
							"table_name" => "notif_log",
	...
	public static function loadAppointment($id = false) {
		if (!$id) return false;
			...
			if(!empty($pat_phone) && $isEnable === false) {
				...
				if (!empty($from_message)) {
					...
					if (!empty($msgId)) {
						...
						$datetime = strtotime('now');
						$msg_date = date('Y-m-d H:i:s', $datetime);
						@$smsObj->logSMS('SMS_MESSAGE', $from_to_phone, $configList->send_phone, $data['pid'], $msgId, $msg_date, $msgStatus, $from_message, 'out', false, $raw_data);
						return true;
	...
	public static function replaceMsgTags($data, $message_text = '') {
		$finalParam = array();
