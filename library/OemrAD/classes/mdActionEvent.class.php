...
class ActionEvent {
	...
	public static function handle15000Threshold1($items = array()) {
		$resultItems = array();
	...
	public static function handle15000Threshold($items = array()) {
		$resultItems = array();
	...
	public static function rpStrVariable($dataStr = '', $params = array()) {
	...
	public static function preProcessingQuery1($ids = array(), $event_type = false) {
		$dataItems = self::getDataForProcess($ids, $event_type);
		$event_ids = array();
		$configList = array();
	...
	public static function sendSMS($data) {
		...
		if(!empty($data)) {
			try {
				...
				$tmp_tosend = isset($data['to_send']) ? $data['to_send'] : '';
				$tosend_list = array();
				if(!empty($tmp_tosend)) {
					$tmp_tosend = explode(",",$tmp_tosend);
					foreach ($tmp_tosend as $i => $toItem) {
						$toItem = trim($toItem);
						if(!in_array($toItem, $tosend_list)) {
							$tosend_list[] = $toItem;
						}
					}
				}

				if(empty($tosend_list)) {
					throw new \Exception("Empty to send list.");
				}

				if(isset($data['config_item']) && isset($data['config_item']['test_mode']) && $data['config_item']['test_mode'] == 1) {
					$data['message'] = "Message To: ". implode(',', $tosend_list) ."<br/>" . $data['message'];
					$tosend_list = Reminder::getTestModeValue("phone");
				}

				$patientValue = isset($data['to_patient']) ? $data['to_patient'] : ' ';
				...
				$pData = self::TransmitSMS(
					array($pItem['data']), 
					array('pid' => $pItem['pid'], 'logMsg' => true)
				);
		...
		if(!empty($data)) {
			try {
				...
				if(!empty($message_list) && isset($message_list->list[$template])) {
					...
					$subject = Reminder::getSubject($data['pid'], $message_list, $data['template_id'], $data['event_datetime'], $data['appt_id']);
				}
	...
	public static function TransmitSMS($pItem = array(), $opts = array()) {
		global $username;
	    ...
	    try {
		    foreach ($pItem as $pk => $phoneData) {
	            $responceData[$pk] = array(
	                'status' => true, 
	                'to' => isset($phoneData['to_send']) ? $phoneData['to_send'] : '', 
	                'errors' => array()
	            );

	            try {
	                if(!isset($phoneData['from']) || empty($phoneData['from'])) {
	                    throw new \Exception('Empty from phone number.');
	                }
	                
	                if(!isset($phoneData['to_send']) || empty($phoneData['to_send'])) {
	                    throw new \Exception('Empty phone number.');
	                }

	                foreach ($phoneData['to_send'] as $toSendItem) {
	                	$smsObj = Smslib::getSmsObj($phoneData['from']);
	                	$result = @$smsObj->smsTransmit($toSendItem, $phoneData['text'], 'text');
	                	$msgId = $result['msgid'];
						...
						$raw_data = json_encode(EmailMessage::includeRequest(
							array(
								'pid' => $pid, 
								'message_tlp' => $phoneData['template'], 
								'phone' => $cphone
							), 
							array(
								'pid',
						...
						$msgLogId = @$smsObj->logSMS('SMS_MESSAGE', $cphone, $phoneData['from'], $pid, $msgId, $msg_date, $msgStatus, $phoneData['text'], 'out', false, $raw_data);

						if(!isset($responceData[($pk)]['data'])) {
                        	$responceData[$pk]['data'] = array();
                    	}
                    	
                    	$responceData[$pk]['data'][] = array('to' => $cphone, 'msgid' => $msgLogId);
	                }
	        ...
	        foreach ($pItem as $pk => $phoneData) {
	            $responceData[$pk] = array(
	                'status' => false, 
	                'to' => isset($phoneData['to_send']) ? $phoneData['to_send'] : array(), 
