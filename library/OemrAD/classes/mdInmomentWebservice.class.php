...
class InmomentWebservice {
	...
	public static function getApiConfigTitle($id = '', $item = array()) {
		...
		if(!empty($item)) {
			...
			if($api_configuration_type == "inmoment") {
				if(isset($item['username']) && !empty($item['username'])) {
					$titleAttrList[] = $item['username'];
				}

				if(isset($item['client_id']) && !empty($item['client_id'])) {
					$titleAttrList[] = $item['client_id'];
				}
	...
	public static function insertExternalAPIConfigurations($data) {
		...
		if($api_config_id) {
			...
			$api_type_sql .= "api_config_id, auth_type, username, password, client_id, client_secret, api_url) VALUES (?, ?, ?, ?, ?, ?, ?) ";
	...
	public static function isNotifyDataExists($event_id, $tablename, $uniqueid, $send_status, $config_id) {
		$row = sqlQuery("SELECT * FROM `vh_inmoment_webservice_notif_log` WHERE event_id = ? AND config_id = ? AND tablename = ? AND uniqueid = ? AND sent = ? ", array($event_id, $config_id, $tablename, $uniqueid, $send_status));
	...
	public static function savePreparedData($data) {
		...
		//Write new record
		$sql = "INSERT INTO `vh_inmoment_webservice_notif_log` ( ";
			...
			foreach ($configs as $key => $action_config) {
				foreach ($action_config['config_data'] as $key => $config_data) {
					...
					if(isset($config['configuration_id']) && !empty($config['configuration_id']) && $config['active'] == "0" && !empty($config['id'])) {
						if($config['action_type'] == "inmoment") {
	...
	public static function getDataForProcess($ids = array(), $event_type = false, $limit = 5000, $offset = 0) {
		$sql = "SELECT * FROM `vh_inmoment_webservice_notif_log` WHERE ";
	...
	public static function getConfiguration($id = '', $type = '', $actionType = '', $eventId = '') {
		...
		$sql .= "aiwc.api_config_id, aiwc.`username` AS username, aiwc.`password` AS password, aiwc.`client_id` AS client_id, aiwc.`client_secret` AS client_secret, aiwc.`api_url` AS api_url ";
		$sql .= "FROM `notification_configurations` nc ";
		$sql .= "LEFT JOIN `vh_api_event_configurations` aec ON aec.`id` = nc.`api_config` ";
		$sql .= "LEFT JOIN `vh_external_api_configurations` as aiwc ON aiwc.api_config_id = aec.id AND aec.api_configuration_type = 'inmoment' ";
	...
	public static function externalApiByEvent($type = 0, $eventid_param = '', $configid_param = '') {
		...
		$iIds = Reminder::getSendItemIdByEvent(array(
			'tablename' => 'vh_inmoment_webservice_notif_log',
		...
		foreach ($configItem as $sk => $sItem) {
			...
			foreach ($chunkDataItems as $cbk => $cBatchItem) {
				...
				if(is_array($batchRecordList) && count($batchRecordList) > 0) {
					...
					$tokenData = self::generateToken($config);

					if (!empty($tokenData) && $tokenData !== false) {
						//Prepare Request
						...
						//Service Call
						$callResponce = self::callRequest($bodyParam, $tokenData);
						...
						if($callResponce) {
							$updateData = array(
								...
								'sent_time' => date('Y-m-d H:i:s'),
								'status' => isset($callResponce['error_description']) ? $callResponce['error_description'] : "success",
	...
	public static function generateToken($data) {
		...
		curl_setopt($ch, CURLOPT_URL, 'https://api.inmoment.com/api/oauth/authorize');
		...
		// Handle result
		$tokenData = self::handle($result, $httpCode);
	...
	public static function updatePreparedData($id, $data) {
		if(!empty($data) && !empty($id)) {
			...
			foreach ($ids as $idk => $id_item) {
				$exiting_result = sqlQuery("SELECT * FROM `vh_inmoment_webservice_notif_log` WHERE id = ?  ", array($id_item));
