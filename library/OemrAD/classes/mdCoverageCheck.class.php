...
class CoverageCheck {
	...
	public static function getConfigVars() {
		...
		$returnList->secret = isset($GLOBALS['availity_client_secret']) ? $cryptoGen->decryptStandard($GLOBALS['availity_client_secret']) : "";
	...
	public static function handleCallVerificationAPI($returnData) {
		$response = array(
			'success' => 0 , 'error' => 'Failed Call Verification'
		);
	...
	public static function callVerificationAPI($insData) {
		...
		$response = array(
			'success' => 0 , 'error' => 'Failed to do api call'
		);
	...
	public static function getCoverageConfiguration($token, $obj) {
		$mapFieldList = array(
			'payerId' => 'alt_cms_id',
		);

		$qtrObj = array('type' => 270);
		...
		$url = "https://api.availity.com/availity/v1/configurations?".$queryString;
		...
		if(isset($responseData['configurations']) && count($responseData['configurations']) > 0 && isset($responseData['configurations'][0]['elements'])) {
			foreach ($responseData['configurations'][0]['elements'] as $eField => $eElementItem) {
				if($eElementItem['required'] == 1) {
					$requiredFieldList[] = $eField;
				}
			}
		}
	...
	public static function getVerificationDataByAPI($token, $obj) {
		...
		if(!$token || !$obj || empty($obj)) {
			return array('success' => 0 , 'error' => 'Invalid token or Invalid request data');
		}

		$requiredFieldList = self::getCoverageConfiguration($token, $obj);
		...
		if(!isset($obj['providerTaxId']) && isset($obj['pr_federaltaxid'])) $obj['providerTaxId'] = $obj['pr_federaltaxid'];
		if(!isset($obj['providerType'])) $obj['providerType'] = isset($GLOBALS['av_default_providerType']) ? $GLOBALS['av_default_providerType'] : '';
		if(!isset($obj['placeOfService'])) $obj['placeOfService'] = isset($GLOBALS['av_default_placeOfService']) ? $GLOBALS['av_default_placeOfService'] : '';

		foreach ($requiredFieldList as $rfItem) {
			if(!isset($qtrObj[$rfItem]) && isset($obj[$rfItem])) {
				$qtrObj[$rfItem] = $obj[$rfItem];
			}
		}
	...
	public static function getVerificationToken($client_id, $client_secret, $scope = 'hipaa') {
		...
		if(!$client_id || !$client_secret) {
			return array('success' => 0 , 'error' => 'Invalid Client Id Or Client Secret value.');
		}
	...
	public static function verifiyData($sourceData, $responceData, $token) {
		$response = array(
			'success' => 0 ,
			'error' => 'Coverage data issue (' . json_encode($responceData) . ')'
		);
		...
		if(isset($sourceData) && isset($responceData)) {
			...
			//	if(count($responceData->coverages) > 0) {
				if (isset($responceData->id) && !empty($responceData->id)) {
					$currentCoverage = isset($responceData) ? $responceData : array();
	...
	/*Get username of user b id*/
	public static function getStatusDifinition($statusCode) {
		if(empty($statusCode)) {
			return "";
		}

		$errorDefinitionList = array(
			"0" => "Availity is in the process of retrieving the coverage from the health plan.",
			"19" => "The health plan has returned one or more validationMessages. (Refer to the validationMessages table.)",
			"R1" => "The health plan did not respond and Availity is retrying the request.",
			"7" => "The health plan did not respond.",
			"13" => "The health plan's response was invalid.",
			"14" => "The health plan did not respond.",
			"15" => "The health plan has indicated that it is down for maintenance.",
			"4" => "The refresh was completed successfully.",
			"3" => "The refresh was completed successfully, but the health plan's response was partially invalid. Availity still returns all available information from the response, but some values may be missing."
		);

		return isset($errorDefinitionList[$statusCode]) ? $errorDefinitionList[$statusCode] : "";
	}
}
