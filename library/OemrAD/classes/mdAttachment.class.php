...
class Attachment {
    ...
    public static function getDocumentDataForSelection($opts = array()) {
        ...
        $query = "SELECT d.id, d.type, d.size, d.url, d.docdate, d.list_id, d.encounter_id, d.foreign_id, d.name as doc_name, d.mimetype, c.name FROM documents AS d, categories_to_documents AS cd, categories AS c WHERE " . $whereStr ." AND cd.document_id = d.id AND c.id = cd.category_id ";
        ...
        while ($drow = sqlFetchArray($dres)) {
            ...
            //Check is file ext exists or not
            $nFileName = $drow['doc_name'];
            if(empty(pathinfo($nFileName, PATHINFO_EXTENSION))) {
                $mExt = self::mime2ext($drow['mimetype']);
                //if(empty($nFileName)) $nFileName = '';
                if(!empty($mExt)) $nFileName = trim($nFileName).".".$mExt;
            }

            $drow['baseName'] = $nFileName . ' (' . xl_document_category($drow['name']). ')';
            $drow['baseFileName'] = '(' . xl_document_category($drow['name']). ')' . $nFileName;
    ...
    public static function prepareMessageAttachment($items) {
        ...
        foreach ($items as $iType => $iItem) {
            if($iType === "documents") {
                ...
                $items[$iType] = $lFiles;
            } else if($iType === "external_files") {
                $lFiles =array();
                foreach ($iItem as $lfKey => $lfItem) {
                    $file_url = isset($lfItem['url']) ? $lfItem['url'] : "";
    ...
    public static function prepareAttachment($pid, $request, $files) {
        try {
            ...
            if((isset($attachList['demos_insurances']) && !empty($attachList['demos_insurances'])) || $request['demoins_inc_demographic'] === "true")  {
                $attachDemosInsData = isset($attachList['demos_insurances']) ? $attachList['demos_insurances'] : array(); 
                $caseInsHtmlContent = self::generateCaseHTML($attachDemosInsData);
            ...
            if(isset($attachList['orders']) && !empty($attachList['orders'])) {
                $orderHtmlContent = self::generateOrderDataAttachment($attachList['orders'], $pid);
            ...
            if(isset($attachList['external_files'])) {
                foreach ($attachList['external_files'] as $key => $fileItem) {
                    if($fileItem['type'] == "file_url") {

                        $fileItem['ignore'] = true;
                        $fileItem['type'] = 'external_files';
                        $fileItem['url'] = $fileItem['url'];
                        $fileItem['name'] = $fileItem['file_name'];
                        $fileItem['file_type'] = $fileItem['file_type'];
            ...
            if(isset($attachList['documents']) && isset($attachList['documents'])) {
                ...
                foreach ($pData['items'] as $pikey => $docObj) {
                    if($docObj['type'] == "file_url") {
                        ...
                        $tmpPath = self::getDocumentFileContent($pid, $docObj['id']);
    ...
    public static function getOrderPDF($orderItems = array(), $pid) {
        $orderHtmlContent = self::generateOrderDataAttachment($orderItems, $pid);

        $bodyHtml = $orderHtmlContent;
        $bodyHtml = self::setHeaderFooterOfHtml('H', $bodyHtml, self::getPdfHeaderContent($pid));
        $bodyHtml = self::setHeaderFooterOfHtml('F', $bodyHtml, '');

        return self::getAndSavePDF(array(
            'pid' => $pid,
            'filename' => 'orders',
            'content' => $bodyHtml,
            'pdf_top_margin' => 2.5,
    ...
    public static function getDocumentFileContent($pid, $docId) {
        ...
        if ($couch_docid && $couch_revid) {
            ...
            if ($d->get_encrypted() == 1) {
                $content = $cryptoGen->decryptStandard($content->data, null, 'database');
            } else {
                $content = base64_decode($content->data);
            }
            ...
            $url = $GLOBALS['OE_SITE_DIR'] . '/documents/' . $from_pathname . '/' . $from_filename;

            if ($d->get_encrypted() == 1) {
                $content = $cryptoGen->decryptStandard(file_get_contents($url), null, 'database');
            } else {
                if (!is_dir($url)) {
        ...
        if(!empty($content)) {
            ...
            $temp_couchdb_url = $GLOBALS['OE_SITE_DIR'] . '/documents/temp/d_' . date("YmdHis") . self::cleanStr($d->get_name());
    ...
    // Clear Temp Attachment files (Attachment).
    public static function clearTempAttachmentFile($attachmentList = array()) {
        try {
            $typeList = array("documents");
            foreach ($attachmentList as $aKey => $aItem) {
                if(isset($aItem['type']) && !empty($aItem['type'])) {
                    if (in_array($aItem['type'], $typeList)) {
                        if(isset($aItem['tmp_path']) && !empty($aItem['tmp_path'])) {
                            if(isset($aItem['org_path']) && !empty($aItem['org_path'])) {
                                if (file_exists($aItem['tmp_path'])) {
                                        unlink($aItem['tmp_path']);
                                        $attachmentList[$aKey]['path'] = $aItem['org_path'];
                                }
                            }
                        }
                    }
                        ...
                        foreach ($insObj as $key => $obj) {
                            ...
                            $htmlStr .= '<tr>';
                                ...
                                $htmlStr .= '<td class="subscriberData" valign="top" width="180">';
                                    ...
                                    if(isset($obj['subscriber_phone_cell'])) {
                                        $htmlStr .= '<span>Phone: '.$obj['subscriber_phone_cell'].'</span><br/>';
                                    }
                                $htmlStr .= '</td>';
    ...
    public static function getCaseListById($id = '') {
        $cases = array();
        $sql = 'SELECT form_cases.*, (SELECT COUNT(*) FROM case_appointment_link AS ca LEFT JOIN openemr_postcalendar_events AS oe ON (ca.pc_eid = oe.pc_eid) WHERE pid = form_cases.pid AND oe.pc_case = form_cases.id) AS enc_count, pd.phone_cell FROM form_cases JOIN patient_data pd on pd.pid = form_cases.pid WHERE ';
        if($type == 'active') $sql .= 'form_cases.closed = 0 AND ';

        if(!empty($id)) {
            $sql .= "form_cases.id = $id AND ";
        }

        $sql .= 'form_cases.activity > 0 ORDER BY form_cases.id DESC';
        ...
        while($row = sqlFetchArray($res)) {
            for($i=1; $i<=3; $i++) {
                if(isset($row['ins_data_id'.$i]) && !empty($row['ins_data_id'.$i])) {
                    ...
                    $insquery .= 'AND ins.`provider` IS NOT NULL AND ins.`provider` > 0 ORDER BY `date` DESC';

                    $binds[] = $row['ins_data_id'.$i];
                    $fres = sqlStatement($insquery, $binds);
                    $insObj = array();
                    while($insrow = sqlFetchArray($fres)) {
                        if(isset($insrow['subscriber_relationship']) && $insrow['subscriber_relationship'] == "self") {
                            $insrow['subscriber_phone_cell'] = isset($row['phone_cell']) ? $row['phone_cell'] : "";
                        }
                        $insObj[] = $insrow;
                    }
    ...
    public static function getAndSavePDF($opt = array()) {
        ...
        $pdf_type = isset($opt['pdf_type']) ? $opt['pdf_type'] : "F";

        $pageCount = $pdfE->page;
        $content_pdf = $pdfE->Output($fullfilename, $pdf_type);
    ...
    public static function generateOrderDataAttachment($orders, $pid) {
            ...
            <?php
                foreach ($rtos as $key => $rto) {
                    ...
                    if(!empty($layoutData) && !empty($layoutData['grp_form_id']) && !empty($rtoData)) {
                        ...
                        ?>
                        <tr class="insRow1" >
                            <td colspan="5" class="cell1" style="border-bottom: 0px;">
                            ...
                            <?php 
                                if (substr($formname, 0, 3) == 'LBF') {
                                    ...
                                    print_r($GLOBALS['incdir'] . "/forms/$formdir/report.php");
                        ...
                        <tr class="insRow1">
                            <td colspan="5" class="cell1" style="border-top: 0px;" align="right">
                                <span>ELECTRONICALLY SIGNED BY <?php echo UserNameFromName($rto['rto_ordered_by']); ?> on <?php echo date('m/d/Y h:i A', strtotime($rto['date'])); ?></span>
                            </td>
                        </tr>
                    <?php
                    } else {
