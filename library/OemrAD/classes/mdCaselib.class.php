...
class Caselib {
	...
	public static function getMedicalDataOfCase($case_id) {
		if($case_id != "") {
			$sql = "SELECT ope.*, u.fname as provider_fname, u.mname as provider_mname, u.lname as provider_lname, TIMESTAMP(pc_eventDate, pc_startTime) as event_date_time from openemr_postcalendar_events ope, users u where ope.pc_aid=u.id and ope.pc_apptstatus not in ('%', '?','x') and ( u.physician_type = 'general_physician' OR u.physician_type = 'PM_physician_1128' ) and ope.pc_case = $case_id"	;
	...
	public static function getPMDataOfCase($case_id) {
		if($case_id != "") {
			$sql = "SELECT ope.*, u.fname as provider_fname, u.mname as provider_mname, u.lname as provider_lname, TIMESTAMP(pc_eventDate, pc_startTime) as event_date_time from openemr_postcalendar_events ope, users u where ope.pc_aid=u.id and ope.pc_apptstatus not in ('%', '?','x') and ( u.physician_type = 'general_physician' OR u.physician_type = 'PM_physician_1128' ) and ope.pc_case = $case_id and u.id IN (20541,23)"	;
	...
	public static function getOrdesByCase($case_id = '', $order_id = '') {
		$dataItems = array();
...
EOF;
	...
	*/
	...
	public static function orderCaseEle($cnt, $rto) {
		global $pid, $newordermode, $rtoCaseReadyOnly;
		...
		if(isset($rto['rto_case']) && !empty($rto['rto_case'])) {
			...
			<div class="caseContainer">
				<div>
					<input name="rto_case_<?php echo $cnt; ?>" id="rto_case_<?php echo $cnt; ?>" type="text" value="<?php echo $rto['rto_case']; ?>" style="width: auto" class="wmtInput wmtFInput <?php echo $rtoCaseReadyOnly ? 'wmtDisabled' : '' ?>" <?php echo $rtoCaseReadyOnly ? 'readonly' : '' ?> onclick="sel_case('<?php echo $pid; ?>', '<?php echo $cnt; ?>');" title="Click to select or add a case for this appointment" />
				</div>
	...
	public static function getAuthorizedCaseId($pid, $encounter, $encounter_date) {
		if(!empty($encounter)) {
