...
class ZoomIntegration {
	...
	public static function getConfigVars() {
		...
		$returnList->account_id = isset($GLOBALS['zoom_account_id']) ? $GLOBALS['zoom_account_id'] : "";;
		$returnList->client_id = isset($GLOBALS['zoom_client_id']) ? $GLOBALS['zoom_client_id'] : "";;
		$returnList->client_secret = isset($GLOBALS['zoom_client_secret']) ? $cryptoGen->decryptStandard($GLOBALS['zoom_client_secret']) : "";
		$returnList->send_details_config_id = isset($GLOBALS['zoom_send_config_id']) ? $GLOBALS['zoom_send_config_id'] : "";;
				...
				if ($performStatus === true) {
					...
					if(!empty($appointmentData) && !empty($appointmentData['username'])) {
						...
						if(isset($createResData['status']) === true) {
							if(isset($res_return) && $res_return === true) {
	...
	public static function createZoomTeleHealthForAppt($appt_id, $appt_data, $force_update = false) {
		$configList = self::getConfigVars();
	...
	public static function handleSendZoomDetails($appt_id, $extraData = '') {
		$configList = self::getConfigVars();
		$send_details_config_id = !empty($configList->send_details_config_id) ? json_decode($configList->send_details_config_id, true) : array();
		$sd_event_id = isset($send_details_config_id['event_ids']) ? $send_details_config_id['event_ids'] : array();
		$sd_config_id = isset($send_details_config_id['config_ids']) ? $send_details_config_id['config_ids']: array();

		if(!isset($sd_config_id) || empty($sd_config_id)) {
			return array('status' => false, 'total_sent_item' => 0, 'total_failed_item' => 0, 'message' => 'Please configure config id to send notification.');
		}
		
		if(!isset($sd_event_id) || empty($sd_event_id)) {
			return array('status' => false, 'total_sent_item' => 0, 'total_failed_item' => 0, 'message' => 'Please configure event id to send notification.');
		}

		$appointmentData = self::getAppoinmentData($appt_id);
		$event_ids = array_map('trim', $sd_event_id);
		
		$configs = Reminder::getActionConfiguration($event_ids);
		$config_ids = array_map('trim', $sd_config_id);
		$notifIdList = array();
		$notifItemList = array();
	...
	/*Handle to send zoom details*/
	public static function handleSendZoomDetails1($appt_id, $extraData = '') {
		$configList = self::getConfigVars();
	...
	public static function handleZoomTelehealthApptDeleteEvent($appt_id) {
	...
	public static function createZoomMeetingForAppt($appt_id, $appt_data, $force_update = false) {
		...
		if(empty($zoomMeetingData)) {
			...
			// Added Co-Host
			if(isset($GLOBALS['zoom_cohost_email']) && !empty($GLOBALS['zoom_cohost_email'])) {
				$body_params['settings'] = array(
					'alternative_hosts' => $GLOBALS['zoom_cohost_email']
				);	
			}
		...
		return array(
			...
			'messages' => $messages,
			'data' => isset($meeting_details) ? $meeting_details : array()
		);
	}
	...
	public static function tokenGenerate($onlyToken = false) {
		$configList = self::getConfigVars();
		...
		curl_setopt($ch, CURLOPT_USERPWD, $configList->client_id . ":" . $configList->client_secret);
		curl_setopt($ch, CURLOPT_POST, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS, "grant_type=account_credentials&account_id=" . $configList->account_id);

		$result = curl_exec($ch);
		$errCode = curl_errno($ch);
		$errText = curl_error($ch);

		$responce = self::handle($result, $httpCode);

		if($onlyToken === true) return isset($responce['access_token']) ? $responce['access_token'] : "";
	...
	public static function curl($data = null, $api_url, $method = null) {
		...
		if(empty(self::$access_token)) {
			self::$access_token = self::tokenGenerate(true);
		}
	...
	public static function getCategoryDetails($pc_catid) {
	...
	public static function getHostEmailList($appt_id, $pc_aid, $start_time) {
		$zaHostEmailList = array();
