...
$resultData = sqlStatement("SELECT ml.*, us.username AS user_username, fm.fax_id, fm.status_code AS fax_status_code, fm.description AS fax_description, pl.letter_id, pl.status_code AS pl_status_code, pl.description AS pl_description, pn.id AS pn_id FROM `message_log` AS ml LEFT JOIN `fax_messages` AS fm ON fm.message_id = ml.id  AND ml.type = 'FAX' LEFT JOIN `postal_letters` AS pl ON pl.message_id = ml.id AND ml.type = 'P_LETTER' LEFT JOIN `gprelations` AS gp ON gp.type1 = '104' AND gp.type2 = '6' AND gp.id1 = ml.id LEFT JOIN `pnotes` AS pn ON pn.title = '".$messageType."' AND pn.id = gp.id2 LEFT JOIN `users` us ON ml.`userid` = us.`id` WHERE ml.type IN ('FAX', 'P_LETTER') AND (pn.id = '' OR pn.id IS NULL) AND ((ml.type = 'FAX' AND fm.status_code NOT IN ('success', 'processing')) OR (ml.type = 'P_LETTER' AND pl.status_code IN ('-1065', '-1005','-1018', '4', '5'))) AND ml.date > DATE_SUB(NOW(), INTERVAL 24 HOUR) ", array());
