...
<html>
<head>
	<title>CronJob - WordPress Sync</title>
</head>
<body>
<?php } ?>
<?php
...
<?php
	//Cron lock file
	$cron_lock = fopen(dirname( __FILE__, 1 )."/cron_wordpress.lock", "w+");
	if (flock($cron_lock, LOCK_EX | LOCK_NB)) { // do an exclusive lock
		...
		if(isset($isEventIdExists) && $isEventIdExists !== false) {
			$sync_info_responce = @WordpressWebservice::externalApiByEvent($event_type, $eventid_param, $configid_param);
		} else {
			$sync_info_responce = @WordpressWebservice::externalApi($event_type);
		}
