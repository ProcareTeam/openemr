...
<?php
	//Cron lock file
	$cron_lock = fopen("./cron_inmomentwebservice.lock", "w+");
	if (flock($cron_lock, LOCK_EX | LOCK_NB)) { // do an exclusive lock
		...
		if(isset($isEventIdExists) && $isEventIdExists !== false) {
			$sync_info_responce = @InmomentWebservice::externalApiByEvent($event_type, $eventid_param, $configid_param);
		} else {
			$sync_info_responce = @InmomentWebservice::externalApi($event_type);
		}
