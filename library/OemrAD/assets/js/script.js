...
// Confirmbox for unverified email ("new_comprehensive.php") [V100050]
...
async function handleBeforeSubmit_NewComprehensive(eleId) {
    ...
    var data = await handleConfimBox_NewComprehensive();
    return data;
}

/*-------------------(Demographics Full)----------------*/

// Confirmbox for demographics ("demographics_full.php") [V100050]
...
$.fn.multielement = function (opts = {}) {
    ...
    this.addElement = function(fieldVals =  {}) {
        if(rawClone) {
            ...
            eleClone.removeClass("h-item hide");
            eleClone.addClass("add_item");
            ...
            if(Object.keys(fieldVals).length > 0) {
                ...
                $(eleClone).find('textarea').val('');
                $(eleClone).find('select').val('');
                $(eleClone).find('input:hidden').val('');
...
async function handlegotoReport(doc_id, pid, setPatient = true) {
    //const pData = await handleSetPatient(pid);
    if(setPatient === true) {
        handleSetPatientData(pid, function() {
    ...
    top.navigateTab(top.webroot_url + '/controller.php?document&view' + "&patient_id=" + pid + "&document_id=" + doc_id + "&","doc0", function () {
        top.activateTabByName("doc0",true);
    });
}
...
async function handleGoToMessage(id, pid, pubpid = '', pname = '', dobstr = '') {
    ...
    top.navigateTab(top.webroot_url + "/interface/main/messages/messages.php?task=edit&noteid="+id, "RTop", function () {
        top.activateTabByName("RTop",true);
    });
}
...
async function handleGoToBalance(pid, otherParam = {}) {
    handleSetPatientData(pid, function() {
        //top.activateTabByName("RTop",true);
    });

    let extraQueryStr = '&' + new URLSearchParams(otherParam).toString();

    top.navigateTab(top.webroot_url + "/interface/reports/idempiere_pat_ledger.php?patient_id=" + pid + "&form=1" + extraQueryStr, "idempiereledger", function () {
       top.activateTabByName("idempiereledger",true);
    });
}
