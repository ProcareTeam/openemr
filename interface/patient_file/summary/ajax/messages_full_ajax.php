...
function getEmailMsg($pid, $draw, $row, $rowperpage, $searchValue, $columnName, $columnSortOrder, $filterVal) {
	...
	while ($email_data = sqlFetchArray($msgRecords)) {
		...
		$formatedMessage = MessagesLib::replaceMessageContent($formatedMessage, $email_data['id']);
		...
		<?php echo MessagesLib::displayAttachment($email_data['type'], $email_data['id'], $email_data, false); ?>
		<?php
		$bodyContent = ob_get_clean();
...
function getSMSMsg($pid, $draw, $row, $rowperpage, $searchValue, $columnName, $columnSortOrder, $filterVal) {
	...
	while ($sms_data = sqlFetchArray($msgRecords)) {
		...
		<?php echo MessagesLib::displayAttachment($sms_data['type'], $sms_data['id'], $sms_data, false); ?>
		<?php
		$bodyContent = ob_get_clean();
...
function getFaxMsg($pid, $draw, $row, $rowperpage, $searchValue, $columnName, $columnSortOrder, $filterVal) {
	...
	while ($fax_data = sqlFetchArray($msgRecords)) {
		...
		<?php echo MessagesLib::displayAttachment($fax_data['type'], $fax_data['id'], $fax_data, false); ?>
		<?php
		$bodyContent = ob_get_clean();
...
function getPostalLetterMsg($pid, $draw, $row, $rowperpage, $searchValue, $columnName, $columnSortOrder, $filterVal) {
	...
	while ($postal_letter_data = sqlFetchArray($msgRecords)) {
		...
		<?php echo MessagesLib::displayAttachment($postal_letter_data['type'], $postal_letter_data['id'], $postal_letter_data, false); ?>
		<?php
		$bodyContent = ob_get_clean();
...
function getChatBoardMsg($pid, $draw, $row, $rowperpage, $searchValue, $columnName, $columnSortOrder, $filterVal) {
	## Search 
	$searchQuery = array();
	if($searchValue != ''){
	   $searchQuery = array();
	...
	## Total number of records without filtering
	$records = sqlQuery("SELECT count(ml.`id`) as allcount FROM `message_log` ml LEFT JOIN `users` us ON ml.`userid` = us.`id` LEFT JOIN `users` u1 ON ml.`assigned` IS NOT NULL AND ml.`assigned` = u1.`id` WHERE ml.`pid` = ? and ml.type = 'CHAT'", array($pid));
	$totalRecords = $records['allcount'];

	## Total number of record with filtering
	$records = sqlQuery("SELECT count(ml.`id`) as allcount FROM `message_log` ml LEFT JOIN `users` us ON ml.`userid` = us.`id` LEFT JOIN `users` u1 ON ml.`assigned` IS NOT NULL AND ml.`assigned` = u1.`id` WHERE ml.`pid` = ? and ml.type = 'CHAT' " .$searchQuery, array($pid));
	$totalRecordwithFilter = $records['allcount'];

	## Fetch records
	$msgQuery = "SELECT ml.*, CONCAT(IFNULL(SUBSTR(us.`fname`,1,1),''), ' ', us.`lname`) AS 'user_name', CONCAT(LEFT(u1.`fname`,1), '. ',u1.`lname`) AS 'assign_user_name' FROM `message_log` ml LEFT JOIN `users` us ON ml.`userid` = us.`id` LEFT JOIN `users` u1 ON ml.`assigned` IS NOT NULL AND ml.`assigned` = u1.`id` WHERE ml.`pid` = ? and ml.type = 'CHAT' ".$searchQuery." order by ".$ordercolumnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;

	$msgRecords = sqlStatement($msgQuery, array($pid));
	$data = array();

	while ($chat_data = sqlFetchArray($msgRecords)) {
		$row_chat_id = $chat_data['id'];
		
		$chat_checked = "";
		$checked = ($chat_data['activity'] == '1') ? "checked" : "";
		
		$user_name = $chat_data['user_name'];
		if (empty($chat_data['userid']) && $chat_data['direction'] == 'in') $user_name = '[ PATIENT ]';
		if (empty($user_name) && $chat_data['direction'] == 'out') $user_name = '[ SYSTEM ]';
		
		$msg_date = $chat_data['msg_time'];
		$datetime = strtotime($chat_data['msg_time']);
		if ($datetime) $msg_date = date('Y-m-d h:ia', $datetime);

		// Get Conversations
		$conversationData = Chat::getChatConversations(array(
			'conversation_id' => array("value" => $chat_data['msg_convid'], "condition" => "")
		), "*");
		...
		if(!empty($conversationData)) {
			...
			$to_from = $name . " - " . implode(", ", $cm);
		}

		ob_start();
		?>
		<div class="btn-group" role="group" >
			<?php if(!empty($chat_data['pid'])) { ?>
			<a href="javascript:;" class='btn btn-secondary btn-sm notes_link' 
				id="<?php echo "unlinkinput".$chat_data['id'] ?>"
				title='<?php echo htmlspecialchars(xl('Chat Message'), ENT_QUOTES) ?>' onClick="unlinkButtonAjax('chat','<?php echo $chat_data['id']; ?>')">
					<span><?php echo htmlspecialchars(xl('Unlink'), ENT_NOQUOTES) ?></span>
			</a>	
			<?php } ?>
			<?php echo generateAssignEle('chat', $chat_data); ?>
			<a href="javascript:;" class='btn btn-secondary btn-sm notes_link' 
				id="<?php echo "openchat".$chat_data['id'] ?>" onClick="openChat('chat','<?php echo $chat_data['msg_convid']; ?>')">
					<span><?php echo htmlspecialchars(xl('Open Chat'), ENT_NOQUOTES) ?></span>
			</a>
		</div>
		<?php
		$actionhtml = ob_get_clean();
		...
		ob_start();
		?>
		<iframe scrolling="no" data-id="<?php echo $chat_data['id']; ?>" class="contentiFrame" srcdoc="<?php echo htmlentities($formatedMessage) ?>"></iframe>
		...
		$data[] = array(
			...
			'active' => $checked,
			'date_time' => $msg_date,
			'assignment' => getassigned('chat', $chat_data),
			'author' => $user_name,
			'to_from' => $to_from,
			'status' => $chat_data['description'] ?? "",
			'content' => $bodyContent
		);
	}

	//PostalLetter::getPostalLetterAddr($chat_data)
	
	//Count Active/Inactive Emails
	$cCount = sqlQuery("SELECT SUM(CASE WHEN activity = 1 THEN 1 ELSE 0 END) AS active, SUM(CASE WHEN activity = 0 THEN 1 ELSE 0 END) AS inactive FROM `message_log` ml WHERE ml.`pid` = ? AND ml.`type` LIKE 'CHAT' " , array($pid));
	...
	$response = array(
	  ...
	  "aaData" => $data,
	  "otherData" => array(
	  	"activeCount" => $cCount['active'],
	  	"inActiveCount" => $cCount['inactive']
	  )
	);
...
} else if($page_action == "chat_board_msg") {
	echo json_encode(getChatBoardMsg($pid, $draw, $row, $rowperpage, $searchValue, $columnName, $columnSortOrder, $filterVal));
}
...
if($mode == "notes_update") {
	...
	$assignNoteId = addPNoteAfterAssing($form_set_id, $form_set_mpid, $form_set_uid, $form_set_username, $form_set_group, $form_set_note_type);
	$set_id = $form_set_id;
	$set_action = $mode;
...
} elseif ($mode == 'chat_update') {
	
	$chat_act = $_POST['chat_act'];
	$chat_chk = $_POST['chat_chk'];
	foreach ($chat_act AS $key => $val) {
		if ($chat_chk[$key] == 'on') {
			sqlStatementNoLog("UPDATE `message_log` SET `activity` = 1 WHERE `id` = ?", array($key));
		} else {
			sqlStatementNoLog("UPDATE `message_log` SET `activity` = 0 WHERE `id`= ?", array($key));
