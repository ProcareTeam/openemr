	...
	<div id="messages">
		<form method='post' name='msg_form' id="msg_form" action='messages_full.php'>
			...
			<ul class="tabNav">
				...
				<li class="<?php if ($active_tab == 'chat') echo 'current' ?>"><a id="header_tab_chat" href="#"><?php echo htmlspecialchars(xl('Chat'),ENT_NOQUOTES); ?></a></li>
			</ul>
			...
			<div class='tabContainer'>
				...
				<!-- CHAT -->
				<div id='chat' class="tab <?php if ($active_tab == 'chat') echo 'current' ?>">
					<div class="pat_notes">
						<table border='0' cellpadding="1" class="text">
						<?php //if (count($postal_letter_list) > 0) { ?>
							<tr>
								<td style="padding: 5px;">
									<div>
										<span><?php echo xl('Total Active CHAT'); ?>: <b><span id="active_Chat">0</span></b></span>
									</div>
									<div>
										<span><?php echo xl('Total Inactive CHAT'); ?>: <b><span id="inactive_chat">0</span></b></span>
									</div>
								</td>
							</tr>
							<tr>
								<td style="padding: 5px;">
									<a href="javascript:;" class="change_activity" onclick="doUpdateAjax('chat')" >
										<span><?php echo htmlspecialchars( xl('Update Active'), ENT_NOQUOTES); ?></span>
									</a>
									&nbsp;|&nbsp;
									<a href="javascript:;" onclick="doRefreshAjax()">
						...
						<table id="chat_table" class="text table tableRowHighLight" style="width:100%">
							<thead>
								<tr>
									<?php
										foreach ($chatboardColumnList as $clk => $cItem) {
											if($cItem["name"] == "dt_control") {
											?> <th><div class="dt-control text"></div></th> <?php
											} else {
											?> <th><?php echo $cItem["title"] ?></th> <?php
...
<script type="text/javascript">
	...
	$(document).ready(function() {
	    ...
	    let chat_dataTable_format = function(d) {
	    ...
	    chat_table = $('#chat_table').dataTable({
	      	...
	      	'serverMethod': 'post',
	      	'ajax': {
	          	url:'ajax/messages_full_ajax.php?pid=<?php echo $pid; ?>&action=chat_board_msg',
	          	data: function(adata) {
          			if(typeof data !== 'undefined') {
	             		for (let key in data) {
	             			adata[key] = data[key];
            ...
            "drawCallback": function (settings) {
            	...
            	$('#active_chat').html(extraData['activeCount'] ? extraData['activeCount'] : 0);
            	$('#inactive_chat').html(extraData['inActiveCount'] ? extraData['inActiveCount'] : 0);

            	//checkSelectedItem('chat_table');

            	setTimeout(function(){
		    		$('#chat_table').find('.contentiFrame').iframereadmoretext();
		    	}, 0);
            },
           	'autoWidth': false,
	      	'columns': prepareColumns('<?php echo json_encode($chatboardColumnList); ?>'),
	      	"columnDefs": [
	      		{
		    		"render": function ( data, type, row ) {
	                    return "<center><input type='hidden' name='selected_chat_act["+row['id']+"]' value='1' /><input type='checkbox' name='selected_chat_chk["+row['id']+"]' "+row['select']+" onClick='handleSelectCheckBox(this,"+row['id']+", \"chat\")'/></center>";
	                },
	                "targets": 1
		      	},
		      	{
		    		"render": function ( data, type, row ) {
	                    return "<center><input type='hidden' name='chat_act["+row['id']+"]' value='1' /><input type='checkbox' name='chat_chk["+row['id']+"]' "+row['active']+"/></center>";
	                },
	                "targets": 2
		      	}
	      	],
		...
		$(chat_table).on('click', 'tbody td.dt-control', function () {
			setTimeout(function(){
				$('.contentiFrame').iframereadmoretext();
			}, 0);
		});
		...
		await $.ajax({
            ...
            success: async function(response) {
            	...
            	if(resStatus === true) {
            		...
            		if(group == "chat") { await chat_table.fnStandingRedraw(); }
            	}
            }
        });
	}
	...
	function doRefreshAjax(paging = '') {
        ...
        if(group == "chat") { chat_table.fnDraw(); }
	}
