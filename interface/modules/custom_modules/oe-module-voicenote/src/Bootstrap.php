...
class Bootstrap
{
    ...
    public function addGlobalModuleSettings(GlobalsInitializedEvent $event)
    {
        ...
        $vnote_section = xlt("Voice Note");
        $service->createSection($vnote_section, 'Portal');
        $vnote_settings = $this->globalsConfig->getGlobalSettingSectionConfiguration();

        foreach ($vnote_settings as $key => $config) {
