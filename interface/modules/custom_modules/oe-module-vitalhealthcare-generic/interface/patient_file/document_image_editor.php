...
// @VH: Update the existing document file. This is specifically used when a user has edited the file using an image editor.

function updateDocument($doc_id, $name, $type, $tmp_name, $error, $size, $higher_level_path = '', $path_depth = '1', $skip_acl_check = false)
{
		try {
	    if(empty($doc_id)) {
	      throw new \Exception("Empty document id " . $tmp_name . ".\n");
	    }
	    ...
	    // Read generated file content for update
	    $tmpfile = fopen($tmp_name, "r");
	    $filetext = fread($tmpfile, $size);
	    fclose($tmpfile);

	    if (empty($mimetype)) {
	    	throw new \Exception("Unable to discover mimetype, so did not upload " . $tmp_name . ".\n");
	    }

	    $d = new Document($doc_id);

      // @VH: Update existing document
      $rc = $d->updateDocument(
          $fname,
          $mimetype,
          $filetext,
      ...
      if (!empty($rc)) {
	    ...
	    return array("doc_id" => $d->id, "url" => $d->url);
	  } catch (\Throwable $e) {
...
if(isset($ajax_action) && !empty($ajax_action)) {
	...
	if($ajax_action == "save_image") {
      ...
      // Generate temp file and save file content into it for update content
			$tempFilePath = tempnam(sys_get_temp_dir(), 'ie');
			...
			// Replace existing file with new file content
			$data = updateDocument(
				$document_id,
        $fname,
        $fileType,
...
async function fetchImage( ajax_action = "get_image", setImage = true, opts = {}) {
	...
	const result = await $.ajax({
        type: "POST",
        url: "<?php echo $GLOBALS['webroot'] .'/interface/modules/custom_modules/oe-module-vitalhealthcare-generic/interface/patient_file/document_image_editor.php'; ?>",
        datatype: "json",
        data: { 
