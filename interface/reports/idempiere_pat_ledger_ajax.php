...
if (!function_exists('prepareChildData')) {
...
if(isset($_REQUEST['page']) && $_REQUEST['page'] == "print") {
    if($_REQUEST['form_extra_payment_filter'] == "charge") {
      ...
      $form_show_patientresponsibilityforcharge = isset($_REQUEST['form_show_patientresponsibilityforcharge']) && $_REQUEST['form_show_patientresponsibilityforcharge'] === "1" ? true : false;

      $rowData = getChargesData($idempiere_connection, false, $_REQUEST['chartNumber'], $_REQUEST['form_extra_case_filter'], $_REQUEST['form_from_date'], $_REQUEST['form_to_date'], 'print', $form_show_patientresponsibilityforcharge);
    ...
    echo '<tbody>';
        ...
        foreach ($rowData as $rowkey => $row) {
            ...
            echo '<tr class="' . ($fRowCount == count($rowData) ? 'lastRow' : '') .'"><td colspan="'.count($colsList).'" class="rowDetails">';
...
<?php
  ...
  if($form_extra_payment_filter == "charge") {
    $rowData = getChargesData($idempiere_connection, false, $chartNumber, $form_extra_case_filter, $form_from_date, $form_to_date, '', $form_show_patientresponsibilityforcharge);
