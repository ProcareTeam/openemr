...
//Get Charge Data
function getChargesData($connection, $entryNumber = false, $chartNumber = '', $caseNumber = '', $dateFrom = '', $dateTo = '', $type = '', $form_show_patientresponsibilityforcharge = false) {
    global $ad_client_id;
    $returnData = array();
    $where = "";
    ...
    if($form_show_patientresponsibilityforcharge === true) {
        $where .= " AND vh_patientresponsibilityforcharge(ci.c_invoiceline_id) >0 ";
    }
...
function getPaymentData($connection, $entryNumber = false, $chartNumber = '', $caseNumber = '', $dateFrom = '', $dateTo = '') {
    ...
    $sql = "SELECT pat.value as chart_number, cc.value as case_number, coalesce(cline.description ,cp.description) as description,  coalesce(cp.datetrx,cline.datetrx) as date_from,prov.name as attending_provider,(select arl.name from ad_ref_list arl where arl.AD_Reference_ID=214 and arl.value like cp.tendertype) as procedure_code, (case when cline.amount>0 then cline.amount when coalesce(cline.pc_adjustmentamt,0) >0 then cline.pc_adjustmentamt when coalesce(cline.pc_takeback,0) >0 then cline.pc_takeback when coalesce(cline.pc_withhold,0)>0 then cline.pc_withhold else 0 end) as amount ,cp.description as dep_description,cp.payamt as payment_amount,coalesce(payer.name,allocbp.name) as payer_name,prov.pc_lastname as last_name, prov.pc_credentials as credentials, cp.documentno as entry_number1,cline.c_allocationline_id as entry_number ,coalesce(cd.name,(case when cline.amount>0 then 'Payment' when coalesce(cline.pc_adjustmentamt,0) >0 then 'Adjustment' when coalesce(cline.pc_takeback,0) >0 then 'TakeBack' when coalesce(cline.pc_withhold,0)>0 then 'Withhold'  else '' end)) as PaymentADjustmentCode from c_allocationline cline left outer join c_bpartner allocbp on cline.c_bpartner_id = allocbp.c_bpartner_id left outer join c_payment cp on cline.c_payment_id = cp.c_payment_id  left outer join c_doctype cd on cp.c_doctype_id = cd.c_doctype_id left outer join c_bpartner payer on cp.c_bpartner_id = payer.c_bpartner_id ,c_invoiceline ci left outer join c_bpartner prov on ci.ProCare_Provider_ID=prov.c_bpartner_id ,c_Bpartner pat,x_mwcase cc,c_allocationhdr hdr where cline.c_allocationhdr_id = hdr.c_allocationhdr_id and hdr.docstatus='CO' and cline.c_invoiceline_id = ci.c_invoiceline_id  and ci.pc_patient_id = pat.c_bpartner_id and ci.x_mwcase_id = cc.x_mwcase_id ".$where." union all SELECT pat.value as chart_number, cc.value as case_number, coalesce(cp.description,'') as description, cp.datetrx as date_from,null as attending_provider,(select arl.name from ad_ref_list arl where arl.AD_Reference_ID=214 and arl.value like cp.tendertype) as procedure_code, cp.payamt as amount ,cp.description as dep_description,cp.payamt as payment_amount,pat.name as payer_name,null as last_name, null as credentials, cp.documentno as entry_number1,cp.c_payment_id as entry_number ,cd.name as PaymentADjustmentCode from c_payment cp left outer join x_mwcase cc on cp.x_mwcase_id = cc.x_mwcase_id ,c_Bpartner pat,c_doctype cd where cc.c_bpartner_id = pat.c_Bpartner_id and cp.c_doctype_id = cd.c_doctype_id and cp.docstatus='CO' AND not exists (select 1 from c_Allocationline cline where cline.c_payment_id=cp.c_payment_id) ".$where." order by date_from desc";
    ...
    if($childColsList && $childColsList != "") {
        ...
        $childrowHTML .= '<thead class="thead-light"><tr>';
            ...
            foreach ($childColsList as $key => $col) {
                $childrowHTML .= '<th class="' . ($fcol == count($childColsList) ? 'lastCol' : '') .'">'.$col['title'].'</th>';
                $fcol++;
            }
        ...
        if(!empty($rowData)) {
            ...
            foreach ($rowData as $key => $row) {
                $childrowHTML .= '<tr class="' . ($fRowCount == count($rowData) ? 'lastRow' : '') .'">';
                $fcol = 1;
                    foreach ($childColsList as $key => $col) {
                        $childrowHTML .= '<td class="' . ($fcol == count($childColsList) ? 'lastCol' : '') .'">'.$row[$col['name']].'</td>';
                        $fcol++;
                    }
                $childrowHTML .= '</tr>';
...
function calculateBalance($connection, $chartNumber, $caseNumber, $totalCaseBalance = false) {
    global $ad_client_id;
    ...
    $sql = "SELECT bp.c_bpartner_id param1, xm.x_mwcase_id param2, xm.value as case_id from c_bpartner bp, x_mwcase xm where bp.c_bpartner_id = xm.c_bpartner_id and bp.value = '".trim($chartNumber)."' and xm.ad_client_id = ".$ad_client_id."";
    ...
    foreach ($mainData as $mk => $mItem) {
        ...
        //Replace data
        $sqlResult4 = pg_query($connection, "select sum(viv.linenetamt) as casebilled, sum(vh_chargeadjustmentamt(viv.c_invoiceline_id)) as caseadj from vh_invoiceline_v viv where viv.x_mwcase_id = ".$param2." and viv.c_bpartner_id = ".$param1."");
        ...
        } else if($totalCaseBalance === true) {
            if(is_array($caseNumber) && !empty($caseNumber)) {
                if(in_array($cNo, $caseNumber)) {
                    $caseBalance += $caseOpenBalance1;
                    $caseBilled1 = (!empty($sqlResultData4) && isset($sqlResultData4->{'casebilled'})) ? $sqlResultData4->{'casebilled'} : 0;
                    $caseBilled += $caseBilled1;

                    $caseAdjAmt2 = (!empty($sqlResultData4) && isset($sqlResultData4->{'caseadj'})) ? $sqlResultData4->{'caseadj'} : 0;
                    $caseAdjAmt += $caseAdjAmt2;

                    $casePaidAmtTotal += ($caseBilled1 - $caseOpenBalance1 - $caseAdjAmt2);

                    $caseUnAllocatedAmt1 = $caseAdjAmt1;
                    $caseUnAllocatedAmt += $caseUnAllocatedAmt1;

                    $patientResponsibility += ($tOpenAmt - $caseUnAllocatedAmt1);
                }
            }
        }
...
function get_idempiere_patient_balance($connection, $pid) {
    $chartNumber = getChartNumber($pid);
    $caseList = getCaseDropdown($connection, $chartNumber);

    $caseNumbers = array();
    foreach ($caseList as $caseItem) {
        $caseValue = isset($caseItem['value']) ? $caseItem['value'] : "";
        if(empty($caseValue)) continue;
        $caseNumbers[] = $caseValue;
    }

    return calculateBalance($connection, $chartNumber, $caseNumbers, true);
}