...
$form_show_patientresponsibilityforcharge = isset($_REQUEST['form_show_patientresponsibilityforcharge']) ? "1" : "0";
...
<head>
    ...
    <script type="text/javascript">
        ...
        <?php if (isset($_REQUEST['default_submit'])) { ?>
 ...
 <tr>
  ...
  <td width='30%'>
    ...
    <table class='text'>
      ...
      <td>
        <select name='form_extra_payment_filter' class='form-control' style="min-width: 110px;" onchange="typeChange(this)">
      ...
      <td>
        <select name='form_extra_case_filter' class='form-control' style="width: 180px;">
            <option value=''><?php echo xla('Please Select'); ?></option>
            <?php
      ...
      <td>
        ...
        <tr>
            <td colspan="4" valign="top">
                <div class="checkbox" id="patientresponsibilityforcharge_container">
                    <label><input type="checkbox" name="form_show_patientresponsibilityforcharge" <?php echo $form_show_patientresponsibilityforcharge === "1" ? 'checked="checked"' : '' ?>  /> <?php echo xla('Show Only Patient Responsible'); ?></label>
                </div>
            </td>
        </tr>
    </table>
...
<script type="text/javascript">
    ...
    var form_show_patientresponsibilityforcharge = '<?php echo $form_show_patientresponsibilityforcharge; ?>';

    dataTableAjax = $.ajax({
        type: 'GET',
        url: "idempiere_pat_ledger_ajax.php?page=datatable&form_from_date="+form_from_date+"&form_to_date="+form_to_date+"&chartNumber="+chartNumber+"&form_extra_payment_filter="+form_extra_payment_filter+"&form_extra_case_filter="+form_extra_case_filter+"&form_show_patientresponsibilityforcharge="+form_show_patientresponsibilityforcharge,
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#med_printbutton').click(function(){
            ...
            printAjax = $.ajax({
                type: 'GET',
                url: "idempiere_pat_ledger_ajax.php?page=print&form_from_date="+form_from_date+"&form_to_date="+form_to_date+"&chartNumber="+chartNumber+"&form_extra_payment_filter="+form_extra_payment_filter+"&form_extra_case_filter="+form_extra_case_filter+"&form_show_patientresponsibilityforcharge="+form_show_patientresponsibilityforcharge,
                success: function(result){
        ...
        // Init
        typeChange(document.querySelector('[name="form_extra_payment_filter"]'), true);
    });

    function typeChange(e, init = false) {
        let typeValue = e.value;
        let elementContainer = document.getElementById('patientresponsibilityforcharge_container');

        if(typeValue == "charge") {
            elementContainer.style.display = "block";
            if(init === false) document.querySelector('[name="form_show_patientresponsibilityforcharge"]').checked = false;
        } else {
