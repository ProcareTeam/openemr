...
if (isset($_GET['genreport'])) {
	...
	if($patientId!='' && $IDcaseId!='')
	{
        ...
        //pg_query($idempiere_connection, $psql) or die("Cannot execute query: $psql\n");
        sqlStatement($sql);
    }
    ...
    $baseUrl .= '://' . $_SERVER['HTTP_HOST'] . "/openemr/";
    ...
    //echo $caseId;
    $loadUrl = $GLOBALS['webroot'] . "/genreport.php?pid=" . $pid . "&catid=" . $catid . "&prdid=" . $prdid . "&frdt=" . $frdt."&fname=cos_san_antonio";
...
if (!AclMain::aclCheckCore('acct', 'rep_a')) {
    //echo (new TwigContainer(null, $GLOBALS['kernel']))->getTwig()->render('core/unauthorized.html.twig', ['pageTitle' => xl("COS")]);
...
<head>
    <title><?php echo xlt('San Antonio COS'); ?></title>
    ...
    <script>
        ...
        function fetchData2(pid) {
            ...
            xhttp.open("GET", "?ajax=1&pid=" + pid + "&csrf_token_form=<?php //echo attr(CsrfUtils::collectCsrfToken()); ?>");
            xhttp.send();
        }
