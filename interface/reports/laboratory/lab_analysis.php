...
if (!$printable){
	...
	echo "<div style='float:right;margin:0 10px'><input type='button' class='btn btn-primary' id='state' value='Toggle On' /></div>";
		...
		<table class='text' style="margin-top: 10px; white-space: nowrap">
			<tr>
				...
				<td style="min-width: 150px;">
					...
					title='yyyy-mm-dd' class='datepicker form-control'> <!-- <img src='../../pic/show_calendar.gif'
...
<?php 	
	...
	echo "<td style='text-align:right;width:100%'><input style='font-size:1em' type='submit' name='submit' value='" . xla('Submit') . "' class='btn btn-primary' /></td>";
	echo "</tr></table>";
	echo "</form>";
...
<?php 
	...
	if(!$printable){
		if(!$nothing && $mode == 'list'){
			...
			echo "<input type='hidden' name='form_from_date' class='datepicker' value='". attr($from_date) . "'>";
			echo "<input type='hidden' name='form_to_date' class='datepicker' value='". attr($to_date) . "'>";
			foreach($_POST['value_code'] as $this_valuecode) {
				echo "<input type='hidden' name='value_code[]' value='". attr($this_valuecode) . "'>";
			}
			echo "<input type='submit' name='print' value='" . xla('Print Preview') . "' class='btn btn-primary' />";
			echo "</form>";
		}
		if ($nothing) {
