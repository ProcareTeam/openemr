	...
	foreach ( $appointments as $appointment ) {
		...
		if ($_POST['form_csvexport']) {
            ...
            if($GLOBALS['wmt::link_appt_ins']) {
			    ...
			    $ins = sqlQuery('SELECT ic.`name` FROM `insurance_data` ope LEFT JOIN `insurance_companies` ic ON ope.`provider` = ic.`id` WHERE ope.`pid` = ? AND ope.`type` = "primary" AND ope.`provider` IS NOT NULL AND ope.`provider` != "" ORDER BY `date` DESC',array($appointment['pid']));
            }
