...
function generateFilterQuery($filterData = array()) {
	...
	if(!empty($filterData)) {
		...
		if(isset($filterData['no_future_chiropractic_appointment']) && $filterData['no_future_chiropractic_appointment'] == '1') {
			$filterQryList[] = " NOT EXISTS (SELECT 1 from users u , list_options lo,openemr_postcalendar_events opp where lo.option_id = u.taxonomy and lo.list_id = 'taxonomy' and u.id = opp.pc_aid and lo.title = 'Chiropractic' and opp.pc_pid=ope.pc_pid and opp.pc_eventdate>CURDATE())";
		}
...
<body class="body_top pre_visit_planning">
	<div class="page-title">
	    <h2><?php echo xlt('Pre-Visit Planning'); ?></h2>
	</div>
	...
	function initTooltip() {
	    ...
	    $('#page_report').popover({
	        ...
	        content: function () {
	            ...
	            let url = "pre_visit_planning_report.php?action=fetch_order_summary&order_id=" + encodeURIComponent(el.dataset.orderid) +
	...
	function initDataTable(id, ajax_url = '', data = {}, columnList = []) {
		...
		if(id && id != "" && ajax_url != '' && data) {
		    ...
		    $('#printbtn').click(function() {
                ...
                printWindow.document.write('<html><head><title><?php echo xlt('Pre-Visit Planning'); ?></title>');
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('.frameDisplay iframe', parent.document).each(function(i, obj) {
			var cElement = $(obj).contents().find('body.pre_visit_planning');
