...
//$reportOnly = isset($_REQUEST['report']) ? $_REQUEST['report'] : '0';
$defaultReport = isset($_REQUEST['default_report']) && $_REQUEST['default_report'] == "1" ? true : false;
$defaultCaseList = isset($_REQUEST['case_list']) && !empty($_REQUEST['case_list']) ? json_decode($_REQUEST['case_list'], true) : array();
...
function generateFilterQuery($filterData = array()) {
	...
	//$filterQryList[] = "if(((ic1.ins_type_code in ('20', '21', '16') OR ic2.ins_type_code in ('20', '21', '16') OR ic3.ins_type_code in ('20', '21', '16'))), 1, 0) = 1"; 
	$caseManager = '';

	if(!empty($filterData)) {
		if(isset($filterData['case_list'])) {
			$filterQryList[] = "fc.id in ('" . implode("','", $filterData['case_list']) . "')";
		}
		...
		if(isset($filterData['case_manager']) && !empty($filterData['case_manager'])) {
			if($filterData['case_manager'] == 'blank') {
				...
				//$filterQryList[] = "length(trim(coalesce(vpcmd.field_value,'')))=0";
				$caseManager = "BLANK";
				...
				$caseManager = isset($filterData['case_manager']) ? $filterData['case_manager'] : "";
		...
		if(isset($filterData['no_future_chiropractic_appointment']) && $filterData['no_future_chiropractic_appointment'] == '1') {
			$filterQryList[] = " NOT EXISTS (SELECT 1 from users u , list_options lo,openemr_postcalendar_events opp where lo.option_id = u.taxonomy and lo.list_id = 'taxonomy' and u.id = opp.pc_aid and opp.pc_case=fc.id and lo.title = 'Chiropractic' and opp.pc_pid=fc.pid and opp.pc_eventdate>CURDATE())";
		}
	}

	if(!empty($caseManager)) {
		if($caseManager == "BLANK") $caseManager = "";
		$filterQryList[] = "vpcmd.field_value = '" . $caseManager . "'";
	}
		...
		if(isset($cItem['name'])) {
			if($cItem['name'] == "case_id") {
				...
				if(!empty($tooltip_html)) {
					$fieldHtml = "<div data-toggle='PopOverItem' data-documentpid='" . $row_item['pid'] . "'><a href=\"#!\" class='$req_class' onclick=\"goParentPid('".$row_item['pid']."');\"><span data-toggle='tooltip' class='tooltip_text $req_class' title=''>". $row_item[$cItem['name']] . "<div class='hidden_content'style='display:none;'>".$tooltip_html."</div></span></a></div>";
				} else {
					$fieldHtml = "<div data-toggle='PopOverItem' data-documentpid='" . $row_item['pid'] . "'><a href=\"#!\" class='linktext $req_class' onclick=\"goParentPid('".$row_item['pid']."');\">". $row_item[$cItem['name']] . "</a></div>";
				}
				...
				$rowData[$cItem['name']] = (isset($fieldHtml) && $fieldHtml != "") ? getHtmlString($fieldHtml) : "";
				continue;
			} else if($cItem['name'] == "pm") {
				$fieldHtml = "";

				$pmData = Caselib::getPMDataOfCase($row_item['case_id']);

				if(!empty($pmData)) {
					$next_appt_time = isset($pmData['event_date_time']) ? date('m/d',strtotime($pmData['event_date_time'])) : "";
					$next_appt_provider_name = "";

					if(isset($pmData['provider_fname']) && !empty($pmData['provider_fname'])) {
						$next_appt_provider_name .= ucfirst(substr($pmData['provider_fname'], 0, 1));
					}

					if(isset($pmData['provider_mname']) && !empty($pmData['provider_mname'])) {
						$next_appt_provider_name .= ucfirst(substr($pmData['provider_mname'], 0, 1));
					}

					if(isset($pmData['provider_lname']) && !empty($pmData['provider_lname'])) {
						$next_appt_provider_name .= ucfirst(substr($pmData['provider_lname'], 0, 1));
					}

					$fieldHtml = "<a href=\"#!\" onclick=\"oldEvt('".$pmData['pc_eid']."');\">".$next_appt_provider_name. " " . $next_appt_time ."</a>";
				} else {
					$fieldHtml = 'N';
				}
				...
				if(isset($row_item['vh_rehabplan']) && !empty($row_item['vh_rehabplan'])) {
					$fieldHtml = str_replace(",", ", ", $row_item['vh_rehabplan']);
				}
				...
				if(isset($row_item['vh_rehabprogress']) && !empty($row_item['vh_rehabprogress'])) {
					$fieldHtml = str_replace(",", ", ", $row_item['vh_rehabprogress']);
				}
				...
				$rowData[$cItem['name']] = (isset($fieldHtml) && $fieldHtml != "") ? $fieldHtml : "";
				continue;
			} else if($cItem['name'] == "action_items") { 
				$fieldHtml = "";

				$aiItem = array();
		        if(!empty($row_item['case_id'])) {
		        	$aiResult = sqlStatement("SELECT * from vh_action_items_details where case_id = ? and status = 'pending' order by id asc", $row_item['case_id']);
...
function generateCaseQuery($data = array(), $isSearch = false) {
	...
	$pi_case_join = " join vh_pi_case_management_details vpcmd on vpcmd.case_id = fc.id and vpcmd.field_name = 'case_manager' and vpcmd.field_index = 0 and vpcmd.isActive = 1 ";
	...
	$ins_company_data_join = " left join insurance_companies ic1 on ic1.id = id1.provider left join insurance_companies ic2 on ic2.id = id2.provider left join insurance_companies ic3 on ic3.id = id3.provider";
...
//Get DataTable Data
function getDataTableData($data = array(), $columns = array(), $filterVal = array(), $fetchCountInfo = false) {
	extract($data);
...
if(!empty($page_action)) {
	...
	if($page_action == "fetch_data") {
		...
		), $colList, $filterVal, $row == "0" ? true : false);

		$response_data = array(
			"draw" => intval($draw),
		  	"recordsTotal" => $datatableDataSet['recordsTotal'] ?? 0,
		  	"recordsFiltered" => $datatableDataSet['recordsFiltered'] ?? 0,
		  	"data" => $datatableDataSet['data']
		);

		echo json_encode($response_data);
		exit();
	} else if($page_action == "fetch_order_summary") {
		$orderId = isset($_REQUEST['order_id']) ? $_REQUEST['order_id'] : "";
		$orderPid = isset($_REQUEST['order_pid']) ? $_REQUEST['order_pid'] : "";

		if(!empty($orderId) && !empty($orderPid)) {
			$ordersData = Caselib::getOrdesByCase('', $orderId);

			if(!empty($ordersData)) {
				$ordersData = $ordersData[0];

				$rto_status_title = isset($ordersData['rto_status_title']) ? $ordersData['rto_status_title'] : "";
				...
				ob_start();
				getRTOSummary($ordersData['id'], $ordersData['pid'], $ordersData);
				$orderSummaryHtml = ob_get_clean();
...
<div class="page-title <?php echo $defaultReport === true ? "hideSection" : ""; ?>">
    <h2><?php echo xlt('Case Manager'); ?></h2>
</div>

<div class="dataTables_wrapper datatable_filter mb-4 <?php echo $defaultReport === true ? "hideSection" : ""; ?>">
	<form id="case_manager_report_filter">
		...
		<div class="form-row">
			<!-- <div class="col-2">
				<div class="form-group">
					<label class="form-check-label"><?php //echo xlt('Date of first visit'); ?>:</label>
					<input type="checkbox" name="patient_with_no_future_app_from" class="form-check-input1" value="1">
				</div>
			</div> -->
			<div class="col-10">
				<div class="form-group">
					<label class="form-check-label"><?php echo xlt('No future chiropractic appointment'); ?>:</label>
					<input type="checkbox" name="no_future_chiropractic_appointment" class="form-check-input1" value="1">
				</div>
			</div>
		</div>
		<div class="form-row">
...
<div id="case_manager_report_container" class="table-responsive removepagination">
...
<script type="text/javascript">
	...
	function initTooltip() {
        ...
        $('#case_manager_report tbody tr').popover({
	        sanitize: false,
	        title: function () {
	            //return this.innerHTML;
	            return xl("Patient Picture");
	        },
	        content: function () {
	            let el = this;
	            ...
	            return '<iframe style="border: 0px; width: 250px; height:250px;" src="<?php echo $GLOBALS['webroot']; ?>/interface/reports/myreports/patient_info_popup.php?patient_id=' + encodeURIComponent(el.dataset.documentpid) + '"></iframe>';
	            //return '<div style="min-width:500px;collectCsrfToken">wTEST DATA</div>';
	        },
	        selector: '[data-toggle="PopOverItem"]',
	        boundary: "window",
	        animation: false,
	        placement: "bottom",
	        trigger: "hover focus",
	    ...
	    $('[data-toggle="PopOverItem"]').on('show.bs.popover', function () {
	    ...
	    $('[data-toggle="PopOverItem"]').on('shown.bs.popover', function () {

	        // set event listeners
	        $('.popover').click(function (e) {
	            $('[data-toggle="PopOverItem"]').popover('hide');
	    ...
	    $('[data-toggle="PopOverItem"]').on('mouseleave', function () {
	        var _this = this;
	        setTimeout(function () {
	    ...
	    $('#case_manager_report').popover({
	        ...
	        content: function () {
	            ...
	            // return '<iframe style="border: 0px" type="image/jpeg" src="<?php echo $GLOBALS['webroot']; ?>/controller.php?document&amp;retrieve&amp;patient_id=' + encodeURIComponent(el.dataset.orderpid) + '&amp;document_id='  + encodeURIComponent(el.dataset.orderid) +  '&amp;as_file=false" onload="iframeLoaded(this)"></iframe>';

	            let el = this;
	            if (typeof el.dataset == 'undefined') {
	                return xl("Report Unavailable");
	            }
	            let url = "case_manager_report.php?action=fetch_order_summary&order_id=" + encodeURIComponent(el.dataset.orderid) +
	                "&order_pid=" + encodeURIComponent(el.dataset.orderpid);
	            let fetchedReport;
	            $.ajax({
	                url: url,
	                method: "GET",
	...
	function format(d, columnList = []) {
		...
		var action_items_val = decodeHtmlString(d.action_items);
		...
		return '<div><table class="row_details_table text table table-sm table-borderless mb-0"><tbody>'+
					...
					'</tr>'+
					'<tr>'+
						'<td width="120">Action Items'+
						'</td>'+
						'<td valign="top">'+
							'<div>'+(action_items_val != "" ? action_items_val : defaultVal)+'</div>'+
						'</td>'+
						'<td width="120">'+
		...
		if(id && id != "" && ajax_url != '' && data) {
			var dTable = $(id).DataTable({
			        ...
			        "drawCallback": function (settings) {
                    	...
                    	const resjson = settings.json; // This contains the server response
                    	
                    	if (currentPageInfo.page == 0 && resjson) {
                    		$('.recordCount').html(resjson.recordsFiltered);
                    	}
		    ...
		    const topHeaderElement = $(id + "_length").parent().parent().find('.col-sm-12.col-md-6');

		    if (topHeaderElement.length > 1) {
		    	$('<div class="pageInfo px-3"><span>Total Records: <span class="recordCount">0</span></span></div>').appendTo($(topHeaderElement[1]));
		    }
		    ...
		    $('#printbtn').click(function() {
                ...
                printWindow.document.write('<html><head><title><?php echo xlt('Case Manager'); ?></title>');
	...
	function getFilterValues(id = '') {
	    ...
	    <?php if(isset($defaultCaseList) && !empty($defaultCaseList)) { ?>
	    	indexed_array['case_list'] = <?php echo json_encode($defaultCaseList) ?>;
		<?php } ?>
	...
	function validateForm() {
		var caseManager = document.querySelector('select[name="case_manager"]').value;

		if(caseManager == '') {
			alert('Please select case manager.');
			return false;
		}
	...
	$(function () {
        ...
        <?php if(isset($defaultReport) && $defaultReport === true) { ?>
        	// Generate default report
        	dataTable.draw();

        	const container = document.querySelector('#case_manager_report_container');
			const observer = new ResizeObserver(function() {
			    var event = new CustomEvent('scrollHeightChange', {detail : { page: 'case_manager_report', scrollHeight: document.querySelector('#case_manager_report_container').scrollHeight, elementId : '<?php echo isset($_REQUEST['report_element']) ? $_REQUEST['report_element'] : "" ?>' }});
