	...
	if(!empty($filterData)) {
		if(isset($filterData['form_type']) && !empty($filterData['form_type'])) {
			...
			if(in_array('all', $filterData['form_type'])) {
				$bFormType = array('email','sms','fax','p_letter','chat');
			} else {
			...
			if(in_array('chat', $bFormType)) {
				// Search value and delete
				if(($typekey = array_search('chat', $bFormType)) !== false) {
				    unset($bFormType[$typekey]);
				}

				$typeWhereStr[] = " (m.type IN ('chat') and exists (select 1 from vh_chat_conversations vcc where  m.`msg_convid` = vcc.conversation_id and vcc.chat_status = '0'))";
			}

			if(!empty($bFormType)) {
				$typeWhereStr[] = "m.type IN('" . implode("','", $bFormType) . "')";
			}

			if(!empty($typeWhereStr)) {
				$filterQryList[] = " ( " . implode(" OR ", $typeWhereStr) . " ) ";
			}
		}
		...
		if(isset($cItem['name'])) {
			if($cItem['name'] == "rec_date_time") {
				...
				if ($row_item['pid']) {
					...
					if($row_item['type'] == "EMAIL" || $row_item['type'] == "SMS" || $row_item['type'] == "CHAT") {
				...
				if (!empty($row_item['assigned'])) {
					$fieldHtml = $row_item['user_name'] ."&nbsp;";
				} else if (!empty($row_item['assign_group'])) {
					// Group data
					$grpData = sqlQuery("SELECT l.title as group_name FROM `list_options` l WHERE l.`option_id` = ? AND l.`list_id` LIKE 'Messaging_Groups'", array($row_item['assign_group']));
					$fieldHtml = "<span style='color:red'>" . $grpData['group_name'] . "</span>";
				} else {
				...
				if($row_item['direction'] == "out") {
					...
					$fieldHtml = isset($row_item['recent_username']) ? ucfirst($row_item['recent_username']) : "";
				}
				...
				if($row_item['type'] == "CHAT" && !empty($row_item['msg_convid'])) {
					$conversationData = Chat::getChatConversations(array(
						'conversation_id' => array("value" => $row_item['msg_convid'], "condition" => "")
					), "*");


					if(!empty($conversationData)) {
						$cm = array();
						$name = $conversationData['first_name'] . " " . $conversationData['last_name'];
						
						if(!empty($conversationData['email'])) {
							$cm[] = $conversationData['email'];
						}

						if(!empty($conversationData['phone_number'])) {
							$cm[] = $conversationData['phone_number'];
						}

						$fieldHtml = $name . " - " . implode(", ", $cm);
					}
					
				}
				
				//print_r(getHtmlString($fieldHtml));
				$rowData[$cItem['name']] = (isset($fieldHtml) && $fieldHtml != "") ? getHtmlString($fieldHtml) : "";
				continue;
				...
				if($row_item['type'] != "NOTES" && $row_item['type'] != "SMS" && !empty($row_item['pid'] && $row_item['type'] != "CHAT")) {
				...
				if($row_item['direction'] == "out" && $row_item['type'] != "CHAT") {	
				...
				if($row_item['type'] == "CHAT") {	
					$action8 = "actionType('doopenchat', this, '" . $row_item['id'] . "', '" . $row_item['type'] . "')";
					$actionHtml8 = "<button type='button' class='btn btn-secondary btn-sm' data-pid='".$row_item['pid']."' data-type='".$row_item['type']."' onClick=\"$action8\">".xlt('Open')."</button>";
				}

				if($row_item['type'] == "EMAIL" && $row_item['direction'] == "in") {
					$action6 = "actionType('doresync', this, '" . $row_item['id'] . "', '" . $row_item['type'] . "')";
					$actionHtml6 = "<button type='button' class='btn btn-secondary btn-sm' data-pid='".$row_item['pid']."' data-type='".$row_item['type']."' onClick=\"$action6\">".xlt('Email ReSync')."</button>";
				}

				$actionHtml6 = '<div class="actiondp dropdown"><div class="labelContainer"><button onclick="dpToggle(this)" class="dropbtn btn btn-secondary btn-sm">'.xlt('More Actions').'</button></div><div class="dropdown-content dpContainer">'.$actionHtml1 . $actionHtml2 . $actionHtml3 . $actionHtml6. $actionHtml8.'</div></div>';
				...
				if($row_item['type'] == "CHAT") {	
					$action8 = "actionType('doopenchat', this, '" . $row_item['msg_convid'] . "', '" . $row_item['type'] . "')";
					$actionHtml8 = "<button type='button' class='btn btn-secondary btn-sm' data-pid='".$row_item['pid']."' data-type='".$row_item['type']."' onClick=\"$action8\">".xlt('Open')."</button>";
				}

				if($row_item['type'] == "EMAIL" && $row_item['direction'] == "in") {	
					$action6 = "openMsgContent('".$row_item['id']."')";
					$actionHtml7 = "<button type='button' class='btn btn-secondary btn-sm' data-pid='".$row_item['pid']."' data-type='".$row_item['type']."' onClick=\"$action6\">".xlt('View Message')."</button>";
				}

				$fieldHtml = '<div class="btn-group">' . $actionHtml4 . $actionHtml5 . $actionHtml41 . $actionHtml7. $actionHtml8 . '</div>';
				...
				if($row_item['direction'] == "in" && $row_item['type'] == "EMAIL") {
					$formatedMessage = MessagesLib::replaceMessageContent($formatedMessage, $row_item['id']);
				}
...
function generateQuery($data = array(), $isSearch = false) {
	...
	if($isSearch === true) {
		//$sql .= " LEFT JOIN `vh_chat_conversations` vcc on m.`type` = 'chat' and  m.`msg_convid` = vcc.conversation_id ";
	} else {
		$sql .= " LEFT JOIN `users` u ON m.`assigned` IS NOT NULL AND m.`assigned` = u.`id` LEFT JOIN `patient_data` p ON m.`pid` IS NOT NULL AND m.`pid` = p.`pid` LEFT JOIN `postal_letters` pl ON m.`id` = pl.`message_id` LEFT JOIN `fax_messages` fm ON m.`id` = fm.`message_id` ";
	}
...
if(!empty($page_action)) {
	if($page_action === "fetch_data") {
		...
		if ($set_id && $set_pid && ($set_action == 'link' || $set_action == 'link_all')) {
			...
			if($msgData['type'] == "CHAT" && !empty($msgData['msg_convid'])) {
				Chat::linkPatient($set_pid, $msgData['msg_convid']);
			}
		...
		if($set_id && $set_action == 'unlink') {
			...
			if($msgData['type'] == "CHAT" && !empty($msgData['msg_convid'])) {
				Chat::linkPatient(0, $msgData['msg_convid']);
			}
		}
		...
		// Resync email content
		if ($set_id && $set_action == 'resync_email') {
			$actionRes['resync_email_status'] = EmailMessage::resyncEmailContent($set_id);
		}
	...
	if(!empty($pid) && !empty($type) && !empty($value)) {
		...
		if(!empty($patData)) {
			if($type == "SMS") {
				...
				$rnPhoneWithContryCode = isset($newPhoneValue['msg_phone']) ? $newPhoneValue['msg_phone'] : "";
				$rPhoneWithContryCode = isset($phoneVal['msg_phone']) ? $phoneVal['msg_phone'] : "";
				$rnSecondaryPhoneWithContryCode =  array_filter(array_map(function($item) { return $item['msg_phone']; }, $secondaryPhoneVal));
				
				//Update
				if(!in_array($rnPhoneWithContryCode, array_merge(array_filter(array($rPhoneWithContryCode)),$rnSecondaryPhoneWithContryCode)) ) {
...
<body>
				...
				<div class="row">
				      ...
				      <div class="form-group">
					    ...
					    <select name="form_type" multiple class="form-control form-control-sm" style="min-height: 110px;">
							...
							<option value='chat'><?php xl('Chat','e') ?></option>
						</select>
					  </div>
				    </div>
  				...
  				<div class="row">
  					<div class="col-12">
        				...
        				if($cnotify['notification'] == 'CHAT') {
						?>
						<button type="button" class="btn btn-secondary btn-sm" onclick="actionType('doopenchat', this, '', 'CHAT')"><?php xl('Open Chat','e'); ?></button>
						<?php } ?>
					</div>
				</div>
  			</div>
	...
	<div id="page_report_container" class="datatable_container table-responsive removepagination">
...
<script type="text/javascript">
	...
	function actionType(type, e, id, mType = '') {
		...
		} else if(type == "doreplychat") {
			var pid = $(e).data('pid');
			replyButton("CHAT", pid, id);
		} else if(type == "doopen") {
		...
		} else if (type == "doopenchat") {
			var msg_type = $(e).data('type');
			var pid = $(e).data('pid');
			openChatTab(msg_type, id, pid);
		} else if (type == "doresync") {
			doresync(id);
		}
	}
	...
	async function actionHandleCall(data, doUpdate = false, doUpdateAll = false) {
		...
		let resync_email_status = res.hasOwnProperty('resync_email_status') ? res['resync_email_status'] : [];
		if(resync_email_status.length > 0) {
			alert(resync_email_status.join("\n"));
		}
	...
	function openChatTab(type, convid = "", pid = "") {
		openChat(type, convid);
	}

	async function doresync(id) {
		var aResponce = await actionHandleCall({
			'action' : 'action_handler',
			'set_id' : id,
			'set_status' : '',
			'set_action' : 'resync_email'
		}, true);
	}
