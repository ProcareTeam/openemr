...
function loadMessage(type, id, pid) {
    ...
    if (type == 'EMAIL_REPLY') openEmailPopup("?pid="+pid+"&msgId="+id+"&action=reply");
}
...
function openChat(type, convid = "") {
    let pUrl = top.webroot_url + '/interface/main/messages/chat_board.php';
    if(convid != "") {
        pUrl += "?convid1=" + convid;
    }

    top.navigateTab(pUrl, "chatboard", function () {
...
async function openMsgContent(msg_id = '') {
    let dTitle = 'Message';
    let url = top.webroot_url + '/interface/main/messages/view_msg_content.php?msg_id=' + msg_id;
    let dialogObj = await dlgopen(url, 'emailPop', 'modal-lg', '800', '', dTitle, {
        allowDrag: false,
        allowResize: false
    });
