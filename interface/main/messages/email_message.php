...
// Default Attachments
$df_attachment = isset($_REQUEST['df_attachment']) && !empty($_REQUEST['df_attachment']) ? json_decode($_REQUEST['df_attachment'], true) : array();
...
if ($form_mode == 'retrieve') {
	...
	try {
		...
		// Added other parameters to fetch future appointment
		$data->loadData(getVals($pat_data->pid), $_SESSION['authUserID']);
...
if ($form_mode == 'transmit') {
	...
	try {
		...
		if($form_message != 'free_text') {
			...
			// Added other parameters to fetch future appointment
			$data->loadData(getVals($pat_data->pid), $_SESSION['authUserID'], false, $pat_appt['pc_eid'], false, $pat_appt['pc_aid']);
...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			<?php } else if(!empty($df_attachment)) { ?>
				attachClassObject.setItemsList(<?php echo json_encode(Attachment::prepareMessageAttachment($df_attachment)); ?>, false);
			<?php } ?>
			<?php } ?>
...
<body class="mx-2">
	...
	<script type="text/javascript">
		...
		function setLegalTeamEmails(caseItem) {
			let emailsAddress = caseItem['email_list'];
			if(emailsAddress != "") {
