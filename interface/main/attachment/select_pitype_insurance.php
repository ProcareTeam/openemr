...
function getCaseData($pid) {
	...
	$result = sqlStatement("SELECT pd.pubpid, fc.* FROM form_cases fc left join patient_data pd on pd.pid = fc.pid WHERE fc.pid  = ? order by fc.id desc", $pid);
	while ($row = sqlFetchArray($result)) {
		$lp_results = sqlStatement("SELECT vpcmd.*, u.email FROM vh_pi_case_management_details vpcmd join users u on u.id = vpcmd.field_value and u.active = 1 WHERE vpcmd.case_id = ? AND vpcmd.field_name = 'lp_contact' ", array($row['id']));
					...
					<?php
						foreach ($piTypeCases as $pik => $piItem) {
							...
							<tr>
								...
								<td><?php echo !empty($piItem['email_list']) ? " - " . $piItem['email_list'] : " - <i>(Empty)</i> " ; ?></td>
							</tr>
							<?php
						}
					?>
