...
$.fn.attachment = function (opts = {}) {
    ...
    this.fileUploaderPrepareFile = function() {
        ...
        let tmpSelectedFileList = $.extend(true, [], this.selectedFileList );

        if(tempThis.demoins_inc_demographic === true) {
            let demosItemIndex = tmpSelectedFileList.map(function(elem, index) {
                if (elem.type == "demos_insurances") 
                    return index;
            }).filter(function(x) {
                return x !== undefined;
            });

            demosItemIndex = demosItemIndex.length > 0 ? demosItemIndex[0] : tmpSelectedFileList.length;

            if(demosItemIndex >= 0) {
                tmpSelectedFileList.splice(demosItemIndex, 0, {'type':'demoins_inc_demographic', 'id':'dl_demoins_inc_demographic', 'text_title':'Report: Patient Demographics only'});
            }
        }
       

        tmpSelectedFileList.forEach(function (item, index) {
            // Skip iteration 
            ...
            if(item.type == "files") {
                ...
                ulItem.appendChild(liItem);
            } else if(item.type == "external_files") {
                let itemData = item.item ? item.item : {};
                let iData = itemData['data'] ? itemData['data'] : {};
                ...
                let liItem = document.createElement('li');
                liItem.className = liClass;
                liItem.innerHTML = "<span><span>" + escape(iData.file_name) + "</span> <span class=\"" + typeClass + "\">File URL</span>&nbsp - </span>";
                liItem.appendChild(removeLink);
                ...
                ulItem.appendChild(liItem);
            } else if(item.type == "demoins_inc_demographic") {
                let incdemoRemoveLink = document.createElement('a');
                incdemoRemoveLink.innerHTML = 'Remove';
                incdemoRemoveLink.href = "javascript:void(0);";
                incdemoRemoveLink.onclick = () => {
                    tempThis.removeDemosIns(item.id);
                }

                //Li items.
                let incdemoLiItem = document.createElement('li');
                incdemoLiItem.className = liClass;

                incdemoLiItem.innerHTML = "<span><a href=\"javascript:void(0);\" >" + removeBackslash(item['text_title']) + "</a>&nbsp;<span class=\"" + typeClass + "\">Demos & Ins</span>&nbsp;- </span>";
                incdemoLiItem.appendChild(incdemoRemoveLink);

                ulItem.appendChild(incdemoLiItem);
        ...
        // Filter hidden items
        let tSelectedFileList = tmpSelectedFileList.filter((item) => {
    ...
    this.removeDemosIns = function(itemId, cItemId = '') {
        ...
        if(itemId == "dl_demoins_inc_demographic") {
            this.demoins_inc_demographic = false;
        }
    ...
    this.getItemsList = function(type) {
        if(["documents", "encounters", "messages", "orders", "encounter_forms", "demos_insurances", "local_files", "external_files"].includes(type)) {
            let items = [];
            this.selectedFileList.forEach(function (item, index) {
                if(item.type == type) {
    ...
    this.getItemsDataList = function(type) {
        if(type == "files") {
            ...
            return items;
        } if(["documents", "encounters", "messages", "orders", "encounter_forms", "local_files", "external_files"].includes(type)) {
            let items = [];
            this.selectedFileList.forEach(function (item, index) {
                if(item.type == type) {
                    if(Array.isArray(item['row_item']) && item['row_item'][0]['data']) {
    ...
    this.prepareFiles = function() {
        ...
        if (opts.onPrepareFiles) {
            let callParam = {
                ...
                external_files: this.getItemsDataList('external_files'),
    ...
    this.appendDataToForm = function(formData) {
        ...
        // organize the external file
        let externalfileItems = this.getItemsDataList('external_files');
        if(externalfileItems) {
            formData.append("external_files", JSON.stringify(externalfileItems));
        }
