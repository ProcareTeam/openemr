...
<head>
	...
	<?php Header::setupHeader(['common', 'opener', 'jquery', 'jquery-ui', 'jquery-ui-base', 'datatables', 'datatables-colreorder', 'datatables-bs', 'oemr_ad']); ?>
	...
	<table id="coverage_history_result" class="coverage_history_result table text table-sm" cellspacing="0" width="100%">
		...
		<tbody>
			...
			foreach ($historyData as $key => $item) {
				...
				$statusDefinition = CoverageCheck::getStatusDifinition($item['coverage_data_obj']['statusCode']);
				...
				<tr data-index-value="<?php echo $key; ?>">
					...
					<td><span class="cellText"><?php echo $item['coverage_data_obj']['status']; ?><span> <?php if(!empty($statusDefinition)) { ?>- <span data-toggle="tooltip2" class="tooltip_text" title=""><i class="fa fa-info-circle" aria-hidden="true"></i><div class="hidden_content text" style=
						"display:none;"><span class="text"><?php echo $statusDefinition; ?></span></div></span><?php } ?></td>
