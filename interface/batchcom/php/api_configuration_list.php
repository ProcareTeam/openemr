...
if (isset($action_mode) && !empty($config_id)  && $action_mode == 'delete') {
	if ($api_type == "wordpress") {
		WordpressWebservice::deleteApiEventConfiguration($config_id);
	} else if ($api_type == "inmoment") {
		InmomentWebservice::deleteApiEventConfiguration($config_id);
	} else {
...
if ($api_type == "inmoment") {
	$configurationList = InmomentWebservice::getApiEventConfiguration('', $api_type);	
} else if ($api_type == "wordpress") {
	$configurationList = WordpressWebservice::getApiEventConfiguration('', $api_type);	
}
	...
	if($api_configuration_type == "idempiere_webservice") {
		...
		<table class="detailsTable">
			...
			<tr>
				...
				<td width="150"><?php echo $item['password'] ? str_repeat('*', strlen($item['password'])) : "" ?></td>
			</tr>
			<tr>
		...
		<?php
	} else if($api_configuration_type == "inmoment") {
		?>
		<table class="detailsTable">
			<tr>
				<td width="100"><b><?php xl('API Url:', 'e'); ?></b></td>
				<td colspan="3"><?php echo $item['api_url'] ? $item['api_url'] : "" ?></td>
			</tr>
			<tr>
				<td width="100"><b><?php xl('Username:', 'e'); ?></b></td>
				<td width="150"><?php echo $item['username'] ? $item['username'] : "" ?></td>
				<td width="100"><b><?php xl('Password:', 'e'); ?></b></td>
				<td width="150"><?php echo $item['password'] ? str_repeat('*', strlen($item['password'])) : "" ?></td>
			</tr>
			<tr>
				<td width="100"><b><?php xl('Client Id:', 'e'); ?></b></td>
				<td width="150"><?php echo $item['client_id'] ? $item['client_id'] : "" ?></td>
				<td width="100"><b><?php xl('Client Secret:', 'e'); ?></b></td>
				<td width="150"><?php echo $item['client_secret'] ? str_repeat('*', strlen($item['client_secret'])) : "" ?></td>
			</tr>
			<tr>
				<td width="100"><b><?php xl('Auth Type:', 'e'); ?></b></td>
				<td width="150"><?php echo $item['auth_type'] ? $item['auth_type'] : "" ?></td>
				<td width="100"></td>
				<td width="150"></td>
			</tr>	
		</table>
		<?php
	} else if($api_configuration_type == "wordpress") {
		?>
		<table class="detailsTable">
			<tr>
				<td width="100"><b><?php xl('API Url:', 'e'); ?></b></td>
			...
			<tr>
				<td width="100"><b><?php xl('API Token:', 'e'); ?></b></td>
				<td colspan="3"><?php echo $item['api_token'] ? $item['api_token'] : "" ?></td>
			</tr>
		</table>
		<?php
	} 
...
function writeConfigurationLine($data) {
	global $selectedItem, $showAll, $totalCheckConfig, $api_configuration_type_List, $api_type;
	...
	if(!empty($data)) {
		foreach ($data as $key => $item) {
			...
			if ($api_type == "inmoment") {
				$title_text = InmomentWebservice::getApiConfigTitle('', $item);
			} else if ($api_type == "wordpress") {
				$title_text = WordpressWebservice::getApiConfigTitle('', $item);
			}
