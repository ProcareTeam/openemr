...
// NOW THIS WILL HAPPEN EVERY TIME IT'S NOT A NEW OR UPDATE OPEN ACTION
	...
	$current_dcode = isset($_REQUEST['tmp_current_diagnosis']) && !empty($_REQUEST['tmp_current_diagnosis']) ? json_decode($_REQUEST['tmp_current_diagnosis'], true) : array();

	// Update
	if($form_mode == 'save') {
		if(isset($current_dcode) && !empty($current_dcode)) {
			$diagForUnlink=GetProblemsWithDiags($pid,"all",$encounter);
			foreach ($diagForUnlink as $dgu) {
				UnLinkDiagnosis($pid,$dgu["id"],$encounter);
			}

			$max1 = $dt['tmp_diag_cnt'];
			$cnt1=1;
			while($cnt1 <= $max1) {
				LinkDiagnosis($pid,$dt['dg_id_'.$cnt1],$encounter,$dt['dg_seq_'.$cnt1]);
				$cnt1++;
			}
		}
	}
		...
		if($auto_post_pqrs && !$is_billed) {
			...
			foreach($pqrs_chks as $key => $pqrs_cpt) {
				...
				if(!$frow['id']) {
					...
					BillingUtilities::addBilling($encounter, 'CPT4', $pqrs_cpt, $desc, $pid, 
	...
	// FIX - This is where we can add tasks
  	...
  	LinkDiagnosis($pid,$dt['dg_id_'.$cnt],$encounter,$dt['dg_seq_'.$cnt]);
		...
		// Delete feesheet code from extended exam form
		$current_diagnosis_codes1 = $dt['tmp_diag_window_mode'] == "current" && isset($_REQUEST['tmp_current_diagnosis']) && !empty($_REQUEST['tmp_current_diagnosis']) ? json_decode($_REQUEST['tmp_current_diagnosis'], true) : array();

		$diag1=GetProblemsWithDiags($pid,$dt['tmp_diag_window_mode'],$encounter, '', false, $current_diagnosis_codes1);

		$cnt1 = 0;
		foreach($diag1 as $prev1) {
			if(isset($prev1['id']) && $prev1['id'] == $dt['dg_id_'.$cnt]) {
				$codes = explode(';',$prev1['diagnosis']);
				...
				if($type && $code) {
					...
					if(billingExists($type, $code, $pid, $encounter)) {
						...
						// Delete billing code from feesheet
						BillingUtilities::deleteBilling($prev1['billing_id']);
					}
				}
			}
  	...
  	if($old['form_id']) {
	...
	} else if($form_mode == 'linklatestallmeds') {
		if($use_meds_not_rx) {
			$unlink1 = GetList($pid, 'medication', $encounter);
		} else {
			$unlink1 = getLinkedPrescriptionsByPatient($pid, $encounter, '=1');
		}
		foreach($unlink1 as $rx) {
  		UnlinkListEntry($pid,$rx['id'],$encounter,$med_list_type);
		}

		if($use_meds_not_rx) {
			$meds1 = GetList($pid, 'medication');
		} else {
	  	$meds1=getPrescriptionsByPatient($pid, "*", '= 1');
		}

		$cnt1 = 1;
		foreach($meds1 as $prev) { 
			if(!$max_med || ($cnt1 <= $max_med)) {
				LinkListEntry($pid, $prev['id'], $encounter, $med_list_type);
			}
			$cnt1++;
		}

	} else if($form_mode == 'linklatestallhist') {
		if($use_meds_not_rx) {
			$unlink1 = GetList($pid, 'med_history', $encounter);
		} else {
			$unlink1 = getLinkedPrescriptionsByPatient($pid, $encounter, '<=0');
		}
		foreach($unlink1 as $rx) {
  		UnlinkListEntry($pid,$rx['id'],$encounter,$med_list_type);
		}
		...
		if($use_meds_not_rx) {
			$med_hist1 = GetList($pid, 'med_history');
		} else {
	  	$med_hist1=getPrescriptionsByPatient($pid, "*", '< 1');
		}
		
		$cnt1 = 1;
		foreach($med_hist1 as $prev) { 
			if(!$max_med_hist || ($cnt1 <= $max_med_hist)) {
				LinkListEntry($pid, $prev['id'], $encounter, $med_list_type);
			}
			$cnt1++;
		}
	} else if($form_mode == 'linklatestallergies') {
		$allergies1=GetList($pid, 'allergy', $encounter);
		foreach ($allergies1 as $prev) {
			UnLinkListEntry($pid,$prev['id'],$encounter,'allergy');
		}

		$allergies1=GetList($pid, 'allergy');
		foreach ($allergies1 as $prev) {
			LinkListEntry($pid, $prev['id'], $encounter, 'allergy');
		}
...
$current_diagnosis_codes = $dt['tmp_diag_window_mode'] == "current" && isset($_REQUEST['tmp_current_diagnosis']) && !empty($_REQUEST['tmp_current_diagnosis']) ? json_decode($_REQUEST['tmp_current_diagnosis'], true) : array();

$diag=GetProblemsWithDiags($pid,$dt['tmp_diag_window_mode'],$encounter, '', false, $current_diagnosis_codes);
foreach($ge_sections as $section) {
...
function set_selected_current_diag(selectedItem, base, wrap, formID, mode) {
	let tcd =  document.forms[0].querySelector("#tmp_current_diagnosis");
	let iicode = [];

	for (var icode in selectedItem) {
		iicode.push(selectedItem[icode]['code_id']);
	}

	tcd.value = JSON.stringify(iicode);

	ToggleDiagWindowMode(base, wrap, formID, mode);
}
</script>
</html>
