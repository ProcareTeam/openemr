...
class Caselib {
	...
	async validate_FutureAppt(pid) {
		if(!confirm("Update today's and future appts and today's encounter.")) {
		} else {
			document.getElementById('form_ufa').value = "1";
		}

		return true;
	}

	async validate_InsStatusVerify(pid) {
		let verify_ins_status = true;
		var sel = document.querySelectorAll('.ins-dropdown');

		for(let selitem of sel) {
			var policy_id = selitem.options[selitem.selectedIndex].value;
			...
			if(policy_id != "") {
			    ...
			    if(jsonPolicy !== false) {
					if(jsonPolicy['ins_type_code_name'] && ["Other HCFA", "Medicaid", "Blue Cross Blue Shield"].includes(jsonPolicy['ins_type_code_name'])) {
						verify_ins_status = false;
					}
				}
			}
		}

		if(verify_ins_status === false) {
			if(!confirm("Please verify eligibility for all major medical insurances before saving case")) {
				return false;
			}
		}
	...
	async send_action_items_reminder(caseId, actionItemsId, pid, ele = null) {
		let otherParam = {};
		if(actionItemsId == 'new') {
			let aiContainer = ele.parentElement.parentElement;

			// Input value
			let ai_action_item_value = aiContainer.querySelector('textarea[name="tmp_case_header_ai_action_item[]"]');
			let ai_owner_value = aiContainer.querySelector('select[name="tmp_case_header_ai_owner[]"]');
			let ai_status_value = aiContainer.querySelector('select[name="tmp_case_header_ai_status[]"]');

			otherParam = {
				ai_action_item : ai_action_item_value.value,
				ai_owner : ai_owner_value.value,
				ai_status : ai_status_value.value
			}
		}
		
		const result = await $.ajax({
			type: "POST",
			url: top.webroot_url + '/interface/forms/cases/ajax/send_action_item_reminder.php',
			datatype: "json",
			data: { 
				action_id : actionItemsId, 
				case_id : caseId, 
				pid : pid, 
				...otherParam 
			}
		...
		if(result != '') {
			...
			if(resultObj && resultObj['refresh'] == true) {
				location.reload();
			}
		}
	}
