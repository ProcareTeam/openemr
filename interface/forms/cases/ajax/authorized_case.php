...
if($type == "appt") {
	...
	if(!empty($case_id)) {
		...
		$msg_prefix = "";
		if(isset($encounterData['supervisor_id']) && !empty($encounterData['supervisor_id'])) {
			$enc_provider = $encounterData['supervisor_id'];
			$msg_prefix = "supervisor ";
		}
		...
		if(isset($caseData['auth_req']) && $caseData['auth_req'] == true) {
			...
			if(isset($case_auth_provider) && !empty($case_auth_provider)) {
				if($case_auth_provider != $enc_provider) {
					$response['status'] = false;
					$response['message'][] = "Warning - This ".$msg_prefix."provider is not authorized for this case.";
				}
			}
		}
	}
