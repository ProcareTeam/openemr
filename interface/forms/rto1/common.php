...
function UpdateRTOExt($thisPid,$item,$num='',$frame='',$stat='',$note='',$resp='',$action='', $dt='', $target='', $by='', $force_msg=false, $repeat=NULL, $stop=NULL, $case=NULL, $stat1 = NULL, $encounter = 0)
{
...
if($mode == 'new') {
	...
	// Update Form Entry
	addRTOFormEntry($pid, $dt['rto_encounter_'.$cnt], $dt['rto_id_'.$cnt]);
...
<html>
	<head>
		...
		async function updateRTOClicked(pid, base, wrap, cnt, id, mode, fieldId, title) {
			...
			if(validateRTO(false)) {
				...
				<?php 
					...
					if(!empty($editmode)) {
						$actionUrl .= "&editmode=" . $editmode;
					}
				?>

				SetScrollTop();
			  	document.forms[0].action=base+'&mode='+mode+'&wrap='+wrap+'&itemID='+cnt + '<?php echo $actionUrl; ?>';
		...
		function ChangeRTOEncounter(e, cnt) {
			let optionElement = e.options[e.selectedIndex];
			let caseId = optionElement.getAttribute('data-case');
			let caseElement = document.getElementById('rto_case_'+cnt);

			if(caseId != "" && caseId != null) {
				caseElement.classList.add("wmtDisabled");
				caseElement.readOnly = true;
			} else {
				caseElement.classList.remove("wmtDisabled");
			...
			<table width="100%" border="0">
				<tr>
					...
					<td class="wmtLabel" style="vertical-align:top;float:right;margin-right: 45px">
						<?php if ($editmode == 'f') { ?>
						<a class="css_button" onClick="dlgclose();" tabindex="-1" ><span><?php xl('Close','e'); ?></span></a>
						<?php } else { ?>
...
function addOrderBtn() {
	...
	var encounterId = "<?php echo isset($encounter) && !empty($encounter) ? '&encounter=' . $encounter : ''; ?>";
	var url = '<?php echo $GLOBALS['webroot']."/interface/forms/rto1/new_order.php" ?>?&pid='+'<?php echo $pid; ?>'+'&frmdir='+'<?php echo $frmdir; ?>'+'&newordermode=true&pop='+'<?php echo $popmode; ?>'+encounterId;
	dlgopen(url,'addOrderPop', 700, 500, '', 'New Order');
}
