...
// Inscode list
$insurancei2 = array();
$res = sqlStatement("SELECT ic.id, ic.name, ic.ins_type_code, itc.`type` as inc_type_code_name from insurance_companies ic left join insurance_type_codes itc on itc.id = ic.ins_type_code where ic.inactive != 1 and itc.`type` in ('Automobile Medical', 'Workers Compensation Health Plan')", array());
...
var addpayeritems = null;
$(document).ready(function() {
    addpayerContainer = $('.add_payer_container').eq(0).clone();
    addpayeritems = $('.add_payer_item').eq(0).clone();
});
</script>
...
function anyPayerUpdated() {
    // Get all input fields
    const payercount = document.getElementById('ipayercount').value;
    const inputFields = document.querySelectorAll('input, select, textarea');

    for (let k in initialFieldValues) {
        let anyFieldUpdated = false;

        for (let k1 in initialFieldValues[k]) {
            let inputField = document.querySelector('[name="' + k1 + '"]');

            if(inputField) {
                // For checkboxes and radios, store their checked state
                if (inputField.type === 'checkbox' || inputField.type === 'radio') {
                    if(initialFieldValues[k][k1] != inputField.checked) {
                        anyFieldUpdated = true;
                    }
                } else {
                    if(initialFieldValues[k][k1] != inputField.value) {
                        anyFieldUpdated = true;
                    }
                }
            }
        ...
        const updatePayerElement = document.querySelector('input[name="' + k + 'updatepayer"]');
        if(updatePayerElement) {
            updatePayerElement.value = anyFieldUpdated;
        }
    };
}
...
function validate() {
 ...
 for (var i = 1; i <= pcount; ++i) {
  ...
  var samess=true;
  var ss_valid=false;
  var updatepayer = f["i" + i + "updatepayer"].value;
  var currProvider = f["i" + i + "provider"];
  var currSelectedProviderText = currProvider ? currProvider.options[currProvider.selectedIndex].text : "";
  ...
  if (subrelat.options[subrelat.selectedIndex].value == "self") {
   if (!samename) {
    // Add confirm message to list
    confirmMessage.push("[" + currSelectedProviderText + "] - " + <?php echo xlj('Subscriber relationship is self but name is different! Is this really OK?'); ?>);
   ...
   if (!samess && ss_valid) {
    // Add confirm message to list
    confirmMessage.push("[" + currSelectedProviderText + "] - " + <?php echo js_escape(xl('Subscriber relationship is self but SS number is different!') . " " . xl("Is this really OK?")); ?>);
   ...
   if (samename) {
    // Add confirm message to list
    confirmMessage.push("[" + currSelectedProviderText + "] - " + <?php echo xlj('Subscriber relationship is not self but name is the same! Is this really OK?'); ?>);
   ...
   if (samess && ss_valid)  {
    // Add confirm message to list
    confirmMessage.push("[" + currSelectedProviderText + "] - " + <?php echo js_escape(xl('Subscriber relationship is not self but SS number is the same!') . " " . xl("Is this really OK?")); ?>);
...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-xs-12">
            <div class="btn-group">
                <button type="button" class="btn btn-default btn-save" id="submit_btn" value="<?php echo xla('Save'); ?>" onclick="validate();">
                    <?php echo xlt('Save'); ?>
                </button>
...
if (! $GLOBALS['simplified_demographics']) {
    ...
    <div id="INSURANCE" class="insuranceEditContainer">
        ...
        <?php } ?>
        <li class="<?php echo empty($insurance_info) ? 'current' : '' ?> add_payer_item"><a href="#"><?php echo xlt("Add Payer"); ?></a></li>
        </ul>
        <?php } else { ?>
        <ul class="tabNav">
        <?php
    ...
    for ($i = 1; $i <= count($insurance_info); $i++) {
     ...
     <!-- OEMR - Change -->
     <div class="tab <?php echo $i == 1 ? 'current' : '' ?> h-auto w-auto <?php echo ($i) == count($insurance_info) ? 'add_payer_container add_payer' : '' ?>">
      <div class="form-row">
        <div class="col-md-6"><!-- start left column -->
          ...
          <input type="hidden" id="i<?php echo attr($i); ?>updatepayer" name="i<?php echo attr($i); ?>updatepayer" class="form-control" value="" />
          ...
          <!-- OEMR - Change -->
          <div class="form-row i<?php echo $i?>claim_number_container cnumber_container"><!-- start nested row -->
            <div class="col-md-3 pb-1 label_custom ">
          ...
          <div class="form-row"><!-- start nested row -->
            ...
            <div class="col-md-9">
              <?php
                ...
                ?>
              <a href="javascript:popUp('../interface/patient_file/summary/browse.php?browsenum=<?php echo attr_url($i); ?>')"
...
<script type="text/javascript">
<?php for ($i=1; $i<=count($insurance_info) + 10; $i++) { ?>
function auto_populate_employer_address<?php echo $i?>(){
 ...
 {
  document.querySelector('[name="i<?php echo $i?>subscriber_fname"]').value='<?php echo attr($result['fname']); ?>';
  document.querySelector('[name="i<?php echo $i?>subscriber_mname"]').value='<?php echo attr($result['mname']); ?>';
  document.querySelector('[name="i<?php echo $i?>subscriber_lname"]').value='<?php echo attr($result['lname']); ?>';
  document.querySelector('[name="i<?php echo $i?>subscriber_street"]').value='<?php echo attr($result['street']); ?>';
  document.querySelector('[name="i<?php echo $i?>subscriber_city"]').value='<?php echo attr($result['city']); ?>';
  document.querySelector('[name="form_i<?php echo $i?>subscriber_state"]').value='<?php echo attr($result['state']); ?>';
  document.querySelector('[name="i<?php echo $i?>subscriber_postal_code"]').value='<?php echo attr($result['postal_code']); ?>'
  <?php if ($result['country_code']) { ?>
    document.querySelector('[name="form_i<?php echo $i?>subscriber_country"]').value='<?php echo attr($result['country_code']); ?>';
    <?php } ?>
  // document.getElementById('i<?php echo $i?>subscriber_phone').value='<?php //echo attr($result['phone_home']); ?>';
  document.querySelector('[name="i<?php echo $i?>subscriber_DOB"]').value='<?php echo attr($result['DOB']); ?>';
  document.querySelector('[name="i<?php echo $i?>subscriber_ss"]').value='<?php echo attr($result['ss']); ?>';
  document.querySelector('[name="form_i<?php echo $i?>subscriber_sex"]').value = '<?php echo attr($result['sex']); ?>';
  document.querySelector('[name="i<?php echo $i?>subscriber_employer"]').value='<?php echo attr($result2['name']); ?>';
  document.querySelector('[name="i<?php echo $i?>subscriber_employer_street"]').value='<?php echo attr($result2['street']); ?>';
  document.querySelector('[name="i<?php echo $i?>subscriber_employer_city"]').value='<?php echo attr($result2['city']); ?>';
  document.querySelector('[name="form_i<?php echo $i?>subscriber_employer_state"]').value='<?php echo attr($result2['state']); ?>';
  document.querySelector('[name="i<?php echo $i?>subscriber_employer_postal_code"]').value='<?php echo attr($result2['postal_code']); ?>';
  <?php if ($result2['country_code']) { ?>
    document.querySelector('[name="form_i<?php echo $i?>subscriber_employer_country"]').value='<?php echo attr($result2['country_code']); ?>';
    <?php } ?>
 }
}
...
$(document).ready(function() {
    ...
    for (var i = 1; i <= payercount; i++) {
        Array.from(inputFields).forEach((input) => {
            ...
            // Check is field part of insurance
            if(inputName != null && inputName.startsWith('i' + i)) {
                if(!initialFieldValues.hasOwnProperty('i' + i)) {
                    initialFieldValues['i' + i] = {};
                }

                // For checkboxes and radios, store their checked state
                if (input.type === 'checkbox' || input.type === 'radio') {
                    initialFieldValues['i' + i][inputName] = input.checked;
                } else {
                    // For other input types, store their values
                    initialFieldValues['i' + i][inputName] = input.value;
                }
            }
        });
    }
...
<script type="text/javascript">
/* OEMR - Change */
// On Insurance value change
$('.insuranceEditContainer').on('change', '.add_payer .ins-provider', function() {
    let insVal = $(this).val();
    let payerCountElement = $('#ipayercount').val();
    let addinsval = $('#i'+payerCountElement+'provider').val();

    let eidval = $(this).attr('id');
    let eidvalm = eidval.match(/(.*i)([\d]+)([a-zA-Z]{1}.*)$/);
    if(eidvalm.length > 3) {
        $('.insuranceEditContainer .tabNav li:nth-child(' + eidvalm[2] + ')').find('a').html($(this).find('option:selected').text());
    }
    
    if(insVal != "" && addinsval != "") {
        let naddpayerContainer = prepareCloneElement(addpayerContainer);
        $('.insuranceEditContainer ul.tabNav').append(prepareCloneItemElement(addpayeritems));

        $(naddpayerContainer).find(".sel2").select2({
            theme: "bootstrap4",
            dropdownAutoWidth: true,
            width: 'resolve',
        <?php require($GLOBALS['srcdir'] . '/js/xl/select2.js.php'); ?>
        ...
        $('.insuranceEditContainer .tabContainer').append(naddpayerContainer); 
    }
});

// Tabbify click event for added payer item 
$('.insuranceEditContainer').on('click', 'ul.tabNav li.add_payer_item a', function() {
...
function prepareCloneItemElement(eleClone = null) {
    var eClone = null;
...
function prepareCloneElement(eleClone = null) {
    ...
    if(eClone) {
        ...
        $(eClone).find('.cnumber_container').each(function(ii, iele) {
            ...
            let eNewClass = eclass.replace(/^(.*i)([\d]+)(claim_number_container.*)$/, "$1"+newPayerCount+"$3");
