...
require_once("$srcdir/user.inc"); // Added 
...
$mode = '';
$notification_options = array("" => 'None',"SMS" => "SMS", "CHAT" => "CHAT");
...
/* OEMR - Asterisk Change for getting extension */
$checkExtension = sqlQuery("SELECT `extension` FROM `user_extension` WHERE `userid` = ? ORDER BY id DESC LIMIT 1", array($_SESSION['authUserID']));

if(!isset($_POST['extension'])) $_POST['extension'] = isset($checkExtension['extension']) ? $checkExtension['extension'] : "";
/* End */
...
$selectedOptions = explode(', ', $checkNotification['notification']);
if(!isset($_POST['notify'])) $_POST['notify'] = isset($checkNotification['notification']) ? explode(', ', $checkNotification['notification']) : "CHAT";
$notify = $_POST["notify"];

if(!isset($_POST['user_status'])) $_POST['user_status'] = isset($checkNotification['user_status']) ?$checkNotification['user_status'] : "offline";
$user_status_val = $_POST["user_status"];
...
if($mode == 'save') {
	...
	/* OEMR - Asterisk Change for storing extension and availabilty */
	...
	if (isset($_POST["notify"])) {
		$selectedOptions1 = implode(', ', $notify);
		if($checkNotification) {
			sqlQuery("UPDATE user_notification SET notification = '$selectedOptions1', user_status = '$user_status_val' WHERE `user_id` = $userid");
	
		} else {
			sqlQuery("INSERT INTO user_notification SET user_id = $userid, notification ='$selectedOptions1', user_status = '$user_status_val'");	
		}
	}

	if(isset($_POST['extension'])) { 
		if($checkExtension) {
			sqlQuery("UPDATE user_extension SET extension = ?  WHERE `userid` = ?", [$_POST["extension"], $userid]);

		} else {
			sqlQuery("INSERT INTO user_extension SET userid = ?, name = ?, username = ?, extension = ?", [$userid, $user_full_name, $username, $_POST["extension"]]);
		}
		?>
		<script>
			if(parent.document) {
				parent.document.querySelector('#extension-using #extension-number').innerText = "<?php echo !empty($_POST["extension"]) ? $_POST["extension"] : "--"; ?>";

				<?php if($_POST["extension"] != NULL ) { ?>
					parent.document.querySelector('#extension-using .badge').classList.remove("badge-danger");
					parent.document.querySelector('#extension-using .badge').classList.add("badge-success");
				<?php } else { ?>
					parent.document.querySelector('#extension-using .badge').classList.remove("badge-success");
					parent.document.querySelector('#extension-using .badge').classList.add("badge-danger");
				<?php } ?>
				
				let event = new Event("extension-update", {bubbles: true}); // (2)
  			parent.document.dispatchEvent(event);

  			let eventupdate = new CustomEvent("chat-update", { detail : {"status" : '<?php echo $user_status_val; ?>', 'notification' : '<?php echo $selectedOptions1; ?>'}, bubbles: true});
  			parent.document.dispatchEvent(eventupdate);
			}
		</script>
		<?php
	}
...
/* OEMR - Asterisk Change for removing number dropdown*/
...
<?php } ?>
<div id="exten_management" style="width: 80%; ?>">
	<table width='100%' border='0' cellpadding='4'>
		<tr>
			<td><b><?php echo xlt('Associate PBX Extension'); ?></b></td>
			<td><b><?php echo xlt('Chat/SMS Notification'); ?></b></td>
			<td><b><?php echo xlt('Chat User Status'); ?></b></td>
		</tr>
		<tr>
			<td width="40%"><input name="extension" id="extension"  class="form-control" type="number" value="<?php echo text($_POST['extension']); ?>" /></td>
			<td><select name="notify[]" id="notify" class="form-control" multiple style="width: 100%;">
				<?php  foreach ($notification_options as $value => $label) {
					echo "<option value='$value'";
					if(in_array($value, $_POST['notify'])) {
			...
			<td>
				<select name="user_status" id="user_status" class="form-control">
					<?php foreach ($user_status_list as $value => $label) {
						echo "<option value='$value'";
						if($value === $_POST['user_status']) {
							echo " selected='selected'";
						}
						echo ">$label</option>";
					}
